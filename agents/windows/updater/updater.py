...
def apply_request()->int:
 ...
 REQUEST_PATH.unlink(missing_ok=True);_write_result("applied",installed_version=plain,rollout_id=request.get("rollout_id"));time.sleep(2);subprocess.run(["schtasks","/Run","/TN",TASK_NAME],check=False,capture_output=True);return 0
def main()->int:
