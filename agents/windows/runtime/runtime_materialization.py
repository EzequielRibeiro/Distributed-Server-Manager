...
def reconcile(config:dict[str,Any],instance_id:str)->dict[str,Any]:
 ...
 normalized=validate_runtime_spec(record,expected_agent_id=str(config.get("agent_id") or ""));_prepare_private_state(normalized);content_files=materialize_content_activation(normalized);_validate_materialization(normalized);materialize_server_settings(normalized);materialize_dynamic_values(normalized);firewall=managed_firewall.reconcile(normalized);adapter=resolve_adapter(normalized);before=adapter.status(normalized);desired=normalized["desired_state"];running=bool(before.get("running") or before.get("active_state")=="active");operation=None
