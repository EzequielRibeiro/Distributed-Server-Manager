...
from runtime_parity import emit_runtime_event
...
def materialize(config:dict[str,Any],spec:dict[str,Any])->dict[str,Any]:
 agent_id=str(config.get("agent_id") or "").strip();normalized=validate_runtime_spec(spec,expected_agent_id=agent_id);emit_runtime_event("INSTANCE_RUNTIME_MATERIALIZING",agent_id=agent_id,instance_id=normalized["instance_id"])
 try:
  operation=_validate_materialization(normalized);record=instance_runtime.register_instance({**normalized,"observed_state":"unknown","materialized":True});event=emit_runtime_event("INSTANCE_RUNTIME_READY",agent_id=agent_id,instance_id=normalized["instance_id"],data={"adapter":normalized["adapter"],"changed":True});return {"spec":normalized,"instance":record,"operation":{"action":"materialize","changed":True,"state":operation},"event":event}
 except Exception as exc:
  emit_runtime_event("INSTANCE_RUNTIME_FAILED",agent_id=agent_id,instance_id=normalized["instance_id"],data={"phase":"materialize","error":str(exc)[:2000]});raise
...
def remove(config:dict[str,Any],instance_id:str)->dict[str,Any]:
 ...
 event=emit_runtime_event("INSTANCE_RUNTIME_REMOVED",agent_id=normalized["agent_id"],instance_id=normalized["instance_id"],data={"changed":True});return {"instance_id":normalized["instance_id"],"stop":stopped,"operation":{"action":"remove","changed":True},"event":event}
