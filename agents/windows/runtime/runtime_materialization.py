...
def materialize(config:dict[str,Any],spec:dict[str,Any])->dict[str,Any]:
 ...
 try:
  _prepare_private_state(normalized);content_files=materialize_content_activation(normalized);operation=_validate_materialization(normalized);properties=materialize_network_properties(normalized);server_settings=materialize_server_settings(normalized);record=instance_runtime.register_instance({**normalized,"observed_state":"unknown","materialized":True});event=emit_runtime_event(_events(),"INSTANCE_RUNTIME_READY",agent_id=agent_id,instance_id=normalized["instance_id"],data={"adapter":normalized["adapter"],"changed":True,"content_files":content_files});return {"spec":normalized,"instance":record,"operation":{"action":"materialize","changed":True,"state":operation,"network_properties":properties,"server_settings":server_settings,"content_files":content_files},"event":event}
...
def reconcile(config:dict[str,Any],instance_id:str)->dict[str,Any]:
 ...
 normalized=validate_runtime_spec(record,expected_agent_id=str(config.get("agent_id") or ""));_prepare_private_state(normalized);content_files=materialize_content_activation(normalized);_validate_materialization(normalized);materialize_server_settings(normalized);firewall=managed_firewall.reconcile(normalized);adapter=resolve_adapter(normalized);before=adapter.status(normalized);desired=normalized["desired_state"];running=bool(before.get("running") or before.get("active_state")=="active");operation=None
