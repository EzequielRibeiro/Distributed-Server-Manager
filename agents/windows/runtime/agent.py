...
def _queue_depth():
 ...
 return {"instance_results":count(STATE_DIR/"instance-results","*.json"),"console_results":count(STATE_DIR/"console-results","*.json"),"file_results":count(STATE_DIR/"file-results","*.json"),"provisioning":count(STATE_DIR/"instance-provisioning","*.request.json"),"game_data":count(STATE_DIR/"game-data-jobs","*.json"),"backup_results":count(STATE_DIR/"backup-results","*.json"),"broadcast_state":count(STATE_DIR/"broadcast-state","*.json"),"runtime_events":len(read_runtime_events(STATE_DIR,limit=1000))}
