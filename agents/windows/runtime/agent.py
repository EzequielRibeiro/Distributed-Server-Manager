...
from dayz_operation_client import clear_result as clear_dayz_operation_result, handle_command as handle_dayz_operation_command, read_result as read_dayz_operation_result
...
def _post(url,payload,headers=None):
 ...
 try:
  with urllib.request.urlopen(request,timeout=20) as response:return json.loads(response.read().decode("utf-8"))
...
def _memory_total_bytes():
 try:
  ...
  if ctypes.windll.kernel32.GlobalMemoryStatusEx(ctypes.byref(state)):return int(state.ullTotalPhys)
...
def _inventory(config):
 ...
 for key,reader in (("update_result",read_update_result),("provisioning_result",read_provisioning_result),("storage_pool_migration_result",read_storage_pool_migration_result),("game_data_result",read_game_data_result),("instance_result",read_instance_result),("native_restart_result",read_native_restart_result),("dayz_operation_result",read_dayz_operation_result),("console_result",read_console_result),("file_result",read_file_result),("resource_result",read_resource_result),("artifact_result",read_artifact_result),("doctor_result",read_doctor_result),("yarax_admin_result",read_yarax_admin_result)):
  value=reader()
...
def heartbeat(config):
 ...
 if isinstance(commands,list):
  ...
  try:config.clear();config.update(_load_config())
  except Exception:pass
 ...
 if isinstance(provisioning_command,dict):stage_provisioning_command(provisioning_command,config_path=CONFIG_PATH)
 ...
 if isinstance(migration_command,dict):stage_storage_pool_migration(migration_command,config_path=CONFIG_PATH)
 migration_state=result.get("storage_pool_migration_state") if isinstance(result.get("storage_pool_migration_state"),dict) else {}
 if str(migration_state.get("status") or "").lower() in {"completed","failed"}:
  mid=migration_state.get("migration_id") or migration_state.get("cleanup_id")
  if mid:clear_storage_pool_migration_result(str(mid))
...
def run_forever():
 ...
 recover_interrupted_operations(config);start_console_stream(config);start_content_update_inventory(config,logger=lambda message:print(message,file=sys.stderr,flush=True));heartbeat_interval=max(10,int(config.get("heartbeat_interval_seconds",DEFAULT_HEARTBEAT_SECONDS)));reconcile_interval=max(5,int(config.get("reconcile_interval_seconds",DEFAULT_RECONCILE_SECONDS)));next_hb=next_rec=0.0
 while True:
  now=time.monotonic()
  if now>=next_rec:
