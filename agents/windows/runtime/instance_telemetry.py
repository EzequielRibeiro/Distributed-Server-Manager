...
def _process_values(pid):
 ...
 handle=kernel.OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION|PROCESS_VM_READ,False,int(pid))
 ...
 try:
  creation=wintypes.FILETIME();exit_time=wintypes.FILETIME();kernel_time=wintypes.FILETIME();user_time=wintypes.FILETIME();cpu_100ns=None;started_epoch=None
  if kernel.GetProcessTimes(handle,ctypes.byref(creation),ctypes.byref(exit_time),ctypes.byref(kernel_time),ctypes.byref(user_time)):
   to_int=lambda ft:(int(ft.dwHighDateTime)<<32)|int(ft.dwLowDateTime);cpu_100ns=to_int(kernel_time)+to_int(user_time);started_epoch=(to_int(creation)-116444736000000000)/10_000_000
  mem=PMC();mem.cb=ctypes.sizeof(PMC);rss=int(mem.WorkingSetSize) if psapi.GetProcessMemoryInfo(handle,ctypes.byref(mem),mem.cb) else None
  return cpu_100ns,rss,started_epoch
 finally:kernel.CloseHandle(handle)
def _cpu_percent(instance_id,cpu_100ns):
 if cpu_100ns is None:return None
 ...
 path.parent.mkdir(parents=True,exist_ok=True);temp=path.with_suffix(".tmp");temp.write_text(json.dumps({"monotonic":now,"cpu_100ns":cpu_100ns}),encoding="utf-8");os.replace(temp,path)
 if not isinstance(previous,dict):return None
 try:
  elapsed=now-float(previous["monotonic"]);delta=int(cpu_100ns)-int(previous["cpu_100ns"])
  return round((delta/10_000_000)/elapsed*100.0,2) if elapsed>0 and delta>=0 else None
...
def _query(config):
 ...
 if not Path(argv[0]).is_absolute():return {}
 try:
...
def collect_instance_telemetry(config:dict[str,Any]):
 ...
 for item in instance_runtime.list_instances(config):
  instance_id=str(item.get("instance_id") or "").strip();record=instance_runtime.get_instance(instance_id) or {};pid=_process_pid(instance_id,record);cpu_ticks,rss,started=_process_values(pid);game=_query(record.get("telemetry") if isinstance(record.get("telemetry"),dict) else {})
  try:view=instance_runtime.status(config,instance_id);state=str(view.get("observed_state") or "unknown").lower();health="healthy" if state=="running" else "degraded" if state in {"starting","failed","unavailable"} else "unknown"
  except Exception:health="unknown"
  result.append({"instance_id":instance_id,"cpu_percent":_cpu_percent(instance_id,cpu_ticks),"memory_bytes":rss,"storage_used_bytes":_storage_used(record.get("path")),"network_rx_bytes":game.get("network_rx_bytes"),"network_tx_bytes":game.get("network_tx_bytes"),"players_online":game.get("players_online"),"players_max":game.get("players_max"),"latency_ms":game.get("latency_ms"),"uptime_seconds":max(0,int(time.time()-started)) if started else None,"health":str(game.get("health") or health)})
 return result
