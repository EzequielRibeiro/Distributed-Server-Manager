...
def _event(event_type:str,instance:dict[str,Any],request:dict[str,Any],data:dict[str,Any]|None=None)->None:emit_runtime_event(Path(instance_runtime.STATE_DIR),event_type,agent_id=instance["agent_id"],instance_id=instance["instance_id"],data={"provisioning_id":request["provisioning_id"],**dict(data or {})})
def _result(path:Path,req:dict[str,Any],*,status:str,current_step:str,progress:int,**extra):payload={"provisioning_id":req["provisioning_id"],"instance_id":req["instance_id"],"status":status,"current_step":current_step,"progress":progress,**extra};write_json(path,payload);return payload
def _validate(config,request):
 ...
 if reserved is not None:
  try:reserved=int(reserved)
  except (TypeError,ValueError) as exc:raise ValueError("storage_reserved_bytes must be an integer") from exc
  if reserved<0:raise ValueError("storage_reserved_bytes cannot be negative")
  if reserved>0 and not str(instance.get("storage_pool_id") or "").strip():raise ValueError("positive storage reservation requires storage_pool_id")
...
def execute(config:dict[str,Any],request:dict[str,Any],result_path:Path)->dict[str,Any]:
 instance=_validate(config,request);step="accepted";started=time.monotonic();materialized=False;_result(result_path,request,status="running",current_step=step,progress=5);_event("INSTANCE_PROVISIONING_STARTED",instance,request)
 try:
  ...
  step="build_runtime_spec";_result(result_path,request,status="running",current_step=step,progress=65);context=dict(request.get("configuration") or {});context["install_path"]=content_result["target_path"];context["content_root"]=content_result["target_path"];context["ports"]=request.get("ports") or context.get("ports") or {};context["desired_state"]=instance.get("desired_state")
  if instance.get("storage_pool_id"):context["storage_pool_id"]=instance["storage_pool_id"]
  spec=game_runtime.build_runtime_spec(config,instance,context);step="materialize_runtime";_result(result_path,request,status="running",current_step=step,progress=82);materialization=runtime_materialization.materialize(config,spec);materialized=True;step="initial_reconcile";_result(result_path,request,status="running",current_step=step,progress=92);reconciliation=runtime_materialization.reconcile(config,instance["instance_id"]);observed=str(reconciliation.get("observed_state") or "unknown")
  final=_result(result_path,request,status="completed",current_step="completed",progress=100,desired_state=spec["desired_state"],observed_state=observed,content={"provider":content_result.get("provider"),"game":content_result.get("game"),"version":content_result.get("version"),"target_path":content_result.get("target_path")},runtime={"profile":spec.get("profile"),"profile_version":spec.get("profile_version"),"adapter":spec.get("adapter"),"runtime_id":spec.get("runtime_id"),"storage_pool_id":spec.get("storage_pool_id"),"instance_state_root":spec.get("instance_state_root"),"materialized_changed":bool((materialization.get("operation") or {}).get("changed"))});increment("provisioning_completed");_event("INSTANCE_PROVISIONING_COMPLETED",instance,request,{"observed_state":observed,"storage_pool_id":spec.get("storage_pool_id")});return final
