...
import game_runtime,runtime_materialization
from provisioning_state import write_json
from runtime_parity import emit_runtime_event,increment
...
def execute(config:dict[str,Any],request:dict[str,Any],result_path:Path)->dict[str,Any]:
 ...
 _result(result_path,request,status="running",current_step=step,progress=5);emit_runtime_event("INSTANCE_PROVISIONING_STARTED",agent_id=instance["agent_id"],instance_id=instance["instance_id"],data={"provisioning_id":request["provisioning_id"]})
 try:
