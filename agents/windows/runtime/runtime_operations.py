"""Crash-consistent operation journal for mutating Windows instance runtime work."""
...
def _root():return Path(instance_runtime.STATE_DIR)/"instance-operations"
def _path(instance_id):return _root()/f"{instance_runtime._token(instance_id,'instance_id')}.json"
...
def read_operation(instance_id)->dict[str,Any]|None:
 try:v=json.loads(_path(instance_id).read_text(encoding="utf-8"))
 except (OSError,ValueError):return None
 return v if isinstance(v,dict) else None
def recover_interrupted_operations(config:dict[str,Any])->list[dict[str,Any]]:
 recovered=[];root=_root();local_agent=str(config.get("agent_id") or "").strip()
 try:paths=sorted(root.glob("*.json"))
 except OSError:return recovered
 for path in paths:
  try:v=json.loads(path.read_text(encoding="utf-8"))
  except (OSError,ValueError):continue
  if not isinstance(v,dict) or v.get("status")!="running" or str(v.get("agent_id") or local_agent)!=local_agent:continue
  v["status"]="interrupted";v["interrupted_at"]=_now();v["error"]="Agent restarted before operation completion";_atomic(path,v);recovered.append(v)
 return recovered
@contextmanager
def runtime_operation(config:dict[str,Any],instance_id:str,operation:str,*,lock_timeout_seconds:float=5.0)->Iterator[dict[str,Any]]:
 ...
 with instance_lock(instance_id,operation,timeout_seconds=lock_timeout_seconds):
  ...
  try:yield journal
   ...
   journal["status"]="failed";journal["error"]=str(exc)[:2000];journal["finished_at"]=_now();journal["duration_ms"]=int((time.monotonic()-started)*1000);_atomic(_path(instance_id),journal);raise
  else:
   journal["status"]="completed";journal["finished_at"]=_now();journal["duration_ms"]=int((time.monotonic()-started)*1000);_atomic(_path(instance_id),journal)
