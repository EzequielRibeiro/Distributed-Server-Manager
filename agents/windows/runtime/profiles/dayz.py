...
class DayZRuntimeProfile(GameRuntimeProfile):
 ...
 def build_runtime_spec(self,instance:dict[str,Any],context:dict[str,Any])->dict[str,Any]:
  ...
  config_path=require_within(install_path,context.get("config_path") or str(Path(working)/"serverDZ.cfg"),"config_path");game_port=require_port(context,"game",protocol="udp")
  ...
  environment={**env,"CAPIVARA_INSTANCE_ID":instance_id,"CAPIVARA_GAME_ID":"dayz","CAPIVARA_GAME_PORT":str(game_port)}
  return {"instance_id":instance_id,"agent_id":agent_id,"game_id":"dayz","environment_id":str(instance.get("environment_id") or "dayz.stable"),"runtime_id":str(instance.get("runtime_id") or instance_id),"adapter":"windows-process","working_directory":working,"executable":executable,"arguments":arguments,"environment":environment,"desired_state":str(instance.get("desired_state") or context.get("desired_state") or "stopped"),"profile":"dayz","profile_version":1,"ports":{"game":{"port":game_port,"protocol":"udp"}},"config_path":config_path}
