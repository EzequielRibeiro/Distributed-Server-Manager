"""Arma 3 Windows runtime profile with private profiles/log state."""
...
class Arma3RuntimeProfile(GameRuntimeProfile):
 ...
 def build_runtime_spec(self,instance:dict[str,Any],context:dict[str,Any])->dict[str,Any]:
  ...
  policy["executable"]=""
  executable=require_within(install,str(Path(install)/"arma3server_x64.exe"),"executable")
  ...
  return {"instance_id":instance_id,"agent_id":agent_id,"game_id":"arma3","environment_id":environment_id,"runtime_id":str(instance.get("runtime_id") or instance_id),"adapter":"windows-process","working_directory":install,"executable":executable,"arguments":args,"environment":{"CAPIVARA_INSTANCE_ID":instance_id,"CAPIVARA_GAME_ID":"arma3"},"desired_state":str(instance.get("desired_state") or context.get("desired_state") or "stopped"),"profile":"arma3","profile_version":1,"ports":ports,"instance_state_root":state,"configuration_root":profiles,"writable_directories":[profiles],"seed_files":[],"seed_directories":[]}
