"""Rust Windows runtime profile with instance-private identity and logs."""
...
class RustRuntimeProfile(GameRuntimeProfile):
 ...
 def build_runtime_spec(self,instance:dict[str,Any],context:dict[str,Any])->dict[str,Any]:
  ...
  policy["executable"]="";policy["arguments"]=[];exe=require_within(install,str(Path(install)/"RustDedicated.exe"),"executable");ports=port_bindings(context)
  ...
  return {"instance_id":iid,"agent_id":aid,"game_id":"rust","environment_id":eid,"runtime_id":str(instance.get("runtime_id") or iid),"adapter":"windows-process","working_directory":runtime,"executable":exe,"executable_scope":"provider-content","seed_source_root":install,"arguments":args,"environment":{"CAPIVARA_INSTANCE_ID":iid,"CAPIVARA_GAME_ID":"rust"},"desired_state":str(instance.get("desired_state") or context.get("desired_state") or "stopped"),"profile":"rust","profile_version":1,"ports":ports,"instance_state_root":state,"configuration_root":str(Path(runtime)/"server"/iid/"cfg"),"writable_directories":[runtime,logs],"seed_files":[],"seed_directories":[]}
