...
def execute(command:dict[str,Any]):
 ...
 target=_target(sel); provider=str(sel.get("provider") or "").lower()
 if action=="verify":
  if not target.is_dir() or not any(target.iterdir()):raise RuntimeError("game-data is not installed")
 ...
 return {"provider":provider,"game":sel.get("game"),"version":sel.get("version"),"target_path":str(target)}
def main():
 if len(sys.argv)!=3:return 2
 req=Path(sys.argv[1]); res=Path(sys.argv[2]); command=json.loads(req.read_text(encoding="utf-8")); job=str(command.get("job_id") or ""); write_json(res,{"job_id":job,"status":"running","progress":5})
 ...
 out={"job_id":job,"status":"completed","progress":100,**detail}; write_json(res,out); sel=command.get("selection") if isinstance(command.get("selection"),dict) else {}; record_game_data(job_id=job,action=str(command.get("action") or "install"),selection=sel,result=out); return 0
