...
def _stabilize(config,iid):
    ...
    if seconds<=0:
        view=status(config,iid);last=str(view.get("observed_state") or "unknown");pid,restarts=_runtime_identity(view)
    ...
    final=status(config,iid);last=str(final.get("observed_state") or "unknown");pid,restarts=_runtime_identity(final)
