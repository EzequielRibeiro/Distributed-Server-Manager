"""Agent-side universal broadcast delivery and durable acknowledgements on Windows."""
...
PROGRAM_DATA=Path(os.environ.get("PROGRAMDATA",r"C:\ProgramData"));STATE_ROOT=Path(os.environ.get("CAPIVARA_AGENT_STATE_DIR",PROGRAM_DATA/"CapivaraAgent"/"state"));STATE_DIR=STATE_ROOT/"broadcast-state"
...
def apply_broadcast_commands(config:dict[str,Any],commands:list[dict[str,Any]])->list[dict[str,Any]]:
 ...
 for cmd in commands[:500]:
  ...
  try:
   ...
   resolve_adapter(instance).broadcast(instance,message,priority=str(cmd.get("priority") or "normal"));now=_now();report={"delivery_id":delivery_id,"broadcast_id":cmd.get("broadcast_id"),"instance_id":iid,"status":"acknowledged" if cmd.get("require_ack",True) else "delivered","delivered_at":now,"acknowledged_at":now if cmd.get("require_ack",True) else None,"last_error":None}
  except Exception as exc:report={"delivery_id":delivery_id,"broadcast_id":cmd.get("broadcast_id"),"instance_id":iid,"status":"failed","delivered_at":None,"acknowledged_at":None,"last_error":str(exc)[:2000]}
