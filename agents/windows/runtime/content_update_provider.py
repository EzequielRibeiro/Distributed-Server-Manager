...
_TOKEN=re.compile(r'^([0-9]+):([0-9]+)$');_TIME_UPDATED=re.compile(r'"timeupdated"\s+"(\d+)"',re.I);_CACHE:dict[str,tuple[float,str]]={}
...
def parse_workshop_manifest(text:str,item_id:str)->str|None:
 ...
 for raw in lines:
  ...
  if pending and not inside:
   ...
   continue
  if inside:
   depth+=s.count('{')-s.count('}');match=_TIME_UPDATED.search(s)
   if match:return match.group(1)
   if depth<=0:return None
 return None
def _manifest_candidates(state_root:Path,app_id:str)->list[Path]:
 name=f'appworkshop_{app_id}.acf';return [state_root/'tools/steamcmd/steamapps/workshop'/name]
...
def upstream_workshop_revision(package_id:str,*,ttl:int=300,opener:Callable[...,Any]=urllib.request.urlopen,force_refresh:bool=False)->str:
 ...
 data=urllib.parse.urlencode({'itemcount':'1','publishedfileids[0]':item_id}).encode('ascii');request=urllib.request.Request('https://api.steampowered.com/ISteamRemoteStorage/GetPublishedFileDetails/v1/',data=data,headers={'User-Agent':'Capivara-Agent/1','Content-Type':'application/x-www-form-urlencoded'})
 with opener(request,timeout=30) as response:payload=json.loads(response.read().decode('utf-8'))
 details=((payload.get('response') or {}).get('publishedfiledetails') or []);revision=str((details[0] if details else {}).get('time_updated') or '').strip()
...
def detect_content_update(state:dict[str,Any],state_root:Path,*,opener=urllib.request.urlopen,force_refresh:bool=False)->dict[str,Any]:
 provider=str(state.get('provider') or '').strip().lower();ctype=str(state.get('content_type') or '').strip().lower();package=str(state.get('package_id') or '').strip();base={'schema_version':1,'provider':provider,'content_type':ctype,'package_id':package or None,'rollback_supported':True}
 ...
 installed=installed_workshop_revision(state_root,package);available=upstream_workshop_revision(package,opener=opener,force_refresh=force_refresh);status='unknown' if not installed else 'up_to_date' if installed==available else 'update_available';return {**base,'detector_supported':True,'state':status,'installed_revision':installed,'available_revision':available}
