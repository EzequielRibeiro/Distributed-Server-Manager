...
def _read_request_summary() -> dict[str, Any]:
    ...
    if not isinstance(request, dict):
        return {}
    return {"desired_version": request.get("desired_version"), "channel": request.get("channel"), "rollout_id": request.get("rollout_id"), "batch_number": request.get("batch_number")}
...
def _verify_manifest(manifest: dict[str, Any], version: str, channel: str) -> None:
    ...
    manifest_channel = str(manifest.get("channel") or "").lower(); expected_channel = "beta" if channel == "beta" else "stable"
    if manifest_channel != expected_channel: raise RuntimeError("package channel mismatch")
...
def _verify_package(package_root: Path, version: str, channel: str, external_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for relative in manifest.get("required_files", []):
        file_path = package_root / str(relative); expected = ((manifest.get("files") or {}).get(relative) or {}).get("sha256")
        if not file_path.is_file() or not expected: raise RuntimeError(f"invalid package file: {relative}")
        if hashlib.sha256(file_path.read_bytes()).hexdigest() != expected: raise RuntimeError(f"internal checksum mismatch: {relative}")
    return manifest
...
def _validate_python(package_root: Path) -> None:
    ...
    files = [runtime / name for name in (
        "agent.py", "capabilities.py", "network_inventory.py", "update_client.py", "update_state.py",
        "local_cli.py", "game_data_client.py", "game_data_executor.py", "game_data_state.py", "instance_runtime.py",
    )] + [package_root / "agent" / "common" / "identity.py", package_root / "agent" / "updater" / "updater.py"]
    completed = subprocess.run(["python3", "-m", "py_compile", *[str(path) for path in files]], capture_output=True, text=True, check=False, timeout=30)
...
def _mapping(package_root: Path) -> list[tuple[Path, Path, int, str]]:
    runtime = package_root / "agent" / "runtime"; common = package_root / "agent" / "common"
    return [
...
def _validate_cli_target() -> tuple[bool, str | None]:
    ...
    target = os.path.realpath(CLI_PATH); expected = os.path.realpath(INSTALL_ROOT / "runtime" / "local_cli.py")
    if target != expected: raise RuntimeError(f"{CLI_PATH} points outside the Capivara Agent installation")
    return True, os.readlink(CLI_PATH)


def _reconcile_cli() -> None:
    CLI_PATH.parent.mkdir(parents=True, exist_ok=True); expected = INSTALL_ROOT / "runtime" / "local_cli.py"; temp = CLI_PATH.with_name(f".{CLI_PATH.name}.{os.getpid()}.new")
    try: temp.unlink()
...
def _validate_installed(version: str, manifest: dict[str, Any], mapping: list[tuple[Path, Path, int, str]]) -> None:
    ...
    if installed != version: raise RuntimeError("post-update VERSION validation failed")
    ...
    for _, destination, _, relative in mapping:
        ...
        if not expected: raise RuntimeError(f"post-update manifest entry missing: {relative}")
        if hashlib.sha256(destination.read_bytes()).hexdigest() != expected: raise RuntimeError(f"post-update checksum mismatch: {relative}")
    if not CLI_PATH.is_symlink(): raise RuntimeError("Agent CLI symlink was not reconciled")
    if os.path.realpath(CLI_PATH) != os.path.realpath(INSTALL_ROOT / "runtime" / "local_cli.py"): raise RuntimeError("Agent CLI symlink target is invalid")
...
def apply_request() -> int:
    ...
    plain_version = version[1:] if version.startswith("v") else version; tag = version if version.startswith("v") else f"v{version}"
    package_name = f"capivara-agent-linux-{plain_version}"; archive_name = f"{package_name}.tar.gz"; manifest_name = f"{package_name}.manifest.json"; base = f"https://github.com/{REPOSITORY}/releases/download/{tag}"
    with tempfile.TemporaryDirectory(prefix="capivara-agent-update-") as temporary:
        work = Path(temporary); archive = work / archive_name; checksum = work / f"{archive_name}.sha256"; external_manifest_path = work / manifest_name
        _download(f"{base}/{archive_name}", archive); _download(f"{base}/{archive_name}.sha256", checksum); _download(f"{base}/{manifest_name}", external_manifest_path)
        expected = checksum.read_text(encoding="utf-8").split()[0].strip().lower(); actual = hashlib.sha256(archive.read_bytes()).hexdigest()
        if expected != actual: raise RuntimeError("release checksum mismatch")
        external_manifest = _load_manifest(external_manifest_path); _verify_manifest(external_manifest, plain_version, channel)
        extract = work / "extract"; extract.mkdir()
        with tarfile.open(archive, "r:gz") as package: _safe_extract(package, extract)
        package_root = extract / package_name; manifest = _verify_package(package_root, plain_version, channel, external_manifest); _validate_python(package_root)
        mapping = _mapping(package_root); cli_existed, old_cli_target = _validate_cli_target(); backup_root = work / "rollback"; snapshots = _snapshot_files(mapping, backup_root)
        try:
            _apply_files(mapping); _reconcile_cli(); _validate_installed(plain_version, manifest, mapping); _restart_agent()
        except Exception:
    ...
    _write_result("applied", installed_version=plain_version, desired_version=plain_version, channel=channel, rollout_id=request.get("rollout_id"), batch_number=request.get("batch_number"), source=f"github-release:{REPOSITORY}@{tag}")
    return 0
