...
def _write_result(status: str, **extra: Any) -> dict[str, Any]:
    ...
    _atomic_json(
        HISTORY_DIR / (datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S%fZ") + ".json"),
        payload,
    )
    return payload
...
def _read_request_summary() -> dict[str, Any]:
    ...
    return {
        key: request.get(key)
        for key in ("desired_version", "channel", "rollout_id", "batch_number")
...
def _load_persistent_identity() -> dict[str, Any]:
    ...
    if str(config.get("pairing_token") or "").strip():
        raise RuntimeError(
            "persistent Agent identity still contains a pairing_token; "
            "relink/recovery required before update"
        )
    return config
...
def _verify_manifest(manifest: dict[str, Any], version: str, channel: str) -> None:
    ...
    if str(manifest.get("channel") or "").lower() != expected:
...
def _validate_python(package_root: Path) -> None:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            (completed.stderr or completed.stdout or "Python validation failed")[:2000]
        )
...
def _mapping(package_root: Path) -> list[tuple[Path, Path, int, str]]:
    ...
    for source in sorted((package_root / "agent" / "runtime").rglob("*.py")):
        ...
        dest = INSTALL_ROOT / source.relative_to(package_root / "agent")
        mode = 0o755 if source.name in {
            "agent.py",
            "local_cli.py",
            "cap_dispatch.py",
            "game_data_executor.py",
            "provisioning_executor.py",
        } else 0o644
    ...
    fixed = [
        ...
        (
            ...
            0o755,
            "agent/privileged/reconcile_runtime_identity.py",
        ),
        (
            package_root / "agent/privileged/uninstall_agent.py",
            INSTALL_ROOT / "privileged/uninstall_agent.py",
            0o755,
        ...
        (
            ...
            SYSTEMD_DIR / "capivara-agent-runtime-identity.service",
            0o644,
            "services/capivara-agent-runtime-identity.service",
        ),
        (
            package_root / "services/capivara-agent-uninstall.path",
            SYSTEMD_DIR / "capivara-agent-uninstall.path",
            0o644,
            "services/capivara-agent-uninstall.path",
        ),
        (
            package_root / "services/capivara-agent-uninstall.service",
            SYSTEMD_DIR / "capivara-agent-uninstall.service",
            0o644,
            "services/capivara-agent-uninstall.service",
        ),
        (
            package_root / "agent/updater/updater.py",
            INSTALL_ROOT / "updater/updater.py",
            0o755,
...
def _daemon_reload() -> None:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            (completed.stderr or completed.stdout or "systemctl daemon-reload failed")[:2000]
        )
...
def _set_uninstall_watch(enabled: bool) -> None:
    ...
    completed = subprocess.run(
        ["systemctl", action, "--now", UNINSTALL_PATH_UNIT],
        capture_output=True,
        text=True,
        check=False,
        timeout=30,
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            (
                completed.stderr
                or completed.stdout
                or f"systemctl {action} uninstall watch failed"
            )[:2000]
        )
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            (
                completed.stderr
                or completed.stdout
                or "runtime identity reconciliation failed"
            )[:2000]
        )
...
def _validate_installed(
    ...
    if (INSTALL_ROOT / "VERSION").read_text(encoding="utf-8").strip() != version:
    ...
    for _, destination, _, relative in mapping:
        ...
        if not expected or hashlib.sha256(destination.read_bytes()).hexdigest() != expected:
            raise RuntimeError(f"post-update checksum mismatch: {relative}")
    if (
        not CLI_PATH.is_symlink()
        or os.path.realpath(CLI_PATH)
        != os.path.realpath(INSTALL_ROOT / "runtime/cap_dispatch.py")
    ):
...
def _heartbeat_ok_since(
    checkpoint: tuple[int | None, int],
...
def _restart_agent(expected_agent_id: str) -> None:
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            (completed.stderr or completed.stdout or "Agent restart failed")[:2000]
        )
    ...
    while time.monotonic() < deadline:
        ...
        if active:
            ...
            raise RuntimeError(
                "Agent service left active state before confirming a post-update heartbeat"
            )
        time.sleep(0.5)
    if not observed_active:
        raise RuntimeError("Agent service did not become active after update")
    raise RuntimeError(
        "Agent did not confirm an accepted post-update heartbeat within 45 seconds"
    )
...
def apply_request() -> int:
    ...
    package_name = f"capivara-agent-linux-{plain}"
    ...
    with tempfile.TemporaryDirectory(prefix="capivara-agent-update-") as temporary:
        ...
        external_path = work / manifest_name
        _download(f"{base}/{archive_name}", archive)
        _download(f"{base}/{archive_name}.sha256", checksum)
        _download(f"{base}/{manifest_name}", external_path)
        if (
            checksum.read_text().split()[0].strip().lower()
            != hashlib.sha256(archive.read_bytes()).hexdigest()
        ):
            raise RuntimeError("release checksum mismatch")

        external = _load_manifest(external_path)
        _verify_manifest(external, plain, channel)
        extract = work / "extract"
        extract.mkdir()
        with tarfile.open(archive, "r:gz") as package:
            _safe_extract(package, extract)
        root = extract / package_name
        manifest = _verify_package(root, plain, channel, external)
    ...
    _write_result(
        "applied",
        installed_version=plain,
        desired_version=plain,
        channel=channel,
