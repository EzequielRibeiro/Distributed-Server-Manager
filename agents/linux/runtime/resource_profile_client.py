#!/usr/bin/env python3
"""Apply Controller-approved Resource Profile changes to Linux instance specs."""
...
STATE=Path(os.environ.get("CAPIVARA_AGENT_STATE_DIR","/var/lib/capivara-agent"));RESULTS=STATE/"resource-results";HISTORY=STATE/"resource-history"
...
def apply(config:dict[str,Any],command:dict[str,Any])->dict[str,Any]:
 ...
 try:
  ...
  # Backward-compatible profile keys are normalized at the Agent boundary.
