...
def reconcile(config:dict[str,Any],instance_id:str)->dict[str,Any]:
 ...
 after=adapter.status(normalized);observed=instance_runtime._observed_state(after,record.get("observed_state"));updated=instance_runtime.register_instance({**record,"observed_state":observed});event_type="INSTANCE_RUNTIME_RECONCILED" if operation else "INSTANCE_RUNTIME_IN_SYNC";event=emit_runtime_event(_state_dir(),event_type,instance_id=normalized["instance_id"],agent_id=normalized["agent_id"],data={"desired_state":desired,"observed_state":observed,"changed":operation is not None,"content_files":content_files});return {"instance_id":normalized["instance_id"],"desired_state":desired,"observed_state":observed,"changed":operation is not None,"operation":operation,"content_files":content_files,"instance":updated,"event":event}
