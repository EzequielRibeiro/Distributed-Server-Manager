...
def apply_policy(spec: dict[str, Any], instance: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    working_root = Path(str(result.get("working_directory") or "")).resolve()
    profile_executable = Path(str(result.get("executable") or ""))
    preserve_private_executable = False
    if profile_executable.is_absolute() and str(working_root):
        try:
            profile_executable.resolve().relative_to(working_root)
