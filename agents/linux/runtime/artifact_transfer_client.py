#!/usr/bin/env python3
"""Move large artifacts over the Agent's outbound Controller connection."""
from __future__ import annotations
import hashlib,http.client,json,os,ssl,shutil
...
def _get(config,transfer_id,destination,expected_size=None,expected_sha=None):
 conn,u=_connection(config);endpoint=(u.path.rstrip("/")+"/api/agent/artifacts/download?"+urlencode({"transfer_id":transfer_id})) or "/api/agent/artifacts/download";headers=_headers(config);conn.request("GET",endpoint,headers=headers);response=conn.getresponse()
