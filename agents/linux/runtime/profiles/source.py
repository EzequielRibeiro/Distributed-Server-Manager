...
_SOURCE_LAYOUTS = {
    "garrysmod": {"game_directory": "garrysmod", "private_generated": ("data", "logs")},
    "left4dead2": {"game_directory": "left4dead2", "private_generated": ("logs",)},
    "teamfortress2": {"game_directory": "tf", "private_generated": ("logs",)},
}
...
class SourceRuntimeProfile(GameRuntimeProfile):
    ...
    game_ids = (
        ...
        "teamfortress2", "teamfortress2.stable",
    )
    profile_version = 1
    ...
    def build_runtime_spec(self, instance: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        environment.update({"CAPIVARA_INSTANCE_ID": instance_id, "CAPIVARA_GAME_ID": game_id})
        ports = port_bindings(context)
        binding = ports.get("game_udp") or ports.get("game")
        if not binding or binding.get("protocol") != "udp":
            raise ProfileError("Source runtime requires a UDP game reservation")
        arguments = ["-port", str(binding["port"]), *arguments]
        instance_state_root = require_absolute(context.get("instance_state_root") or f"/var/lib/capivara-instances/{instance_id}", "instance_state_root")
        ...
        bind_paths.extend({"source": str(private_game_root / value), "target": str(shared_game_root / value)} for value in generated)
        return {
            "instance_id": instance_id, "agent_id": agent_id, "game_id": game_id,
            ...
            "profile": "source", "profile_version": self.profile_version, "ports": ports,
            "instance_state_root": instance_state_root, "configuration_root": str(private_cfg),
            "writable_directories": writable_directories, "seed_files": [],
            "seed_directories": [{"source": str(shared_cfg), "target": str(private_cfg)}], "bind_paths": bind_paths,
        }
