...
def _preserve_owner(temp: Path, owner: tuple[int, int] | None) -> None:
    ...
    current = temp.stat()
    if (current.st_uid, current.st_gid) != owner:os.chown(temp, owner[0], owner[1])


def _write(path: Path, payload: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True);owner=_existing_owner(path);temp=path.with_name(f".{path.name}.{os.getpid()}.tmp")
    try:
        temp.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8");os.chmod(temp,0o600);_preserve_owner(temp,owner);os.replace(temp,path)
    finally:
        try:temp.unlink()
...
def list_instances(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for path in paths:
        ...
        if item and str(item.get("agent_id") or "")==local_agent:
            try:adapter_state=_adapter_state(item)
            except (AdapterError,OSError,RuntimeError,ValueError):adapter_state=None
            values.append({"instance_id":item.get("instance_id"),"game_id":item.get("game_id"),"environment_id":item.get("environment_id"),"adapter":item.get("adapter"),"observed_state":_observed_state(adapter_state,item.get("observed_state","unknown"))})
    return values
...
def doctor(config: dict[str, Any], instance_id: str) -> dict[str, Any]:
    ...
    severities={item["severity"] for item in findings};state="critical" if "critical" in severities else "degraded" if "warning" in severities else "healthy"
    return {"schema_version":2,"kind":"CapivaraInstanceDoctor","scope":"instance-local","status":state,"ready":state!="critical","instance":view,"adapter_doctor":adapter_doctor,"findings":findings}
def save(config:dict[str,Any],instance_id:str)->dict[str,Any]:
    ...
    with runtime_operation(config,instance_id,"maintenance:save",lock_timeout_seconds=limits.lock_timeout_seconds):
        record=_owned(config,instance_id);adapter=resolve_adapter(record);result=adapter.save(record);increment("maintenance_save")
        return {"schema_version":1,"kind":"CapivaraInstanceSave","scope":"instance-local","instance_id":record["instance_id"],"agent_id":record["agent_id"],"adapter":adapter.name,"operation":result}
def lifecycle(config: dict[str, Any], instance_id: str, action: str) -> dict[str, Any]:
    ...
    with runtime_operation(config,instance_id,f"lifecycle:{action}",lock_timeout_seconds=limits.lock_timeout_seconds):
        record=_owned(config,instance_id);firewall=None
...
def remove(config: dict[str, Any], instance_id: str) -> dict[str, Any]:
    ...
    from runtime_metrics import increment
    from runtime_operations import runtime_operation
    instance_id=_token(instance_id,"instance_id");limits=runtime_limits(config)
...
def inventory(config:dict[str,Any])->list[dict[str,Any]]:return list_instances(config)
def _history(command_id:str)->Path:return HISTORY_DIR/f"{_token(command_id,'command_id')}.json"
def _result(command_id:str)->Path:return RESULT_DIR/f"{_token(command_id,'command_id')}.json"
