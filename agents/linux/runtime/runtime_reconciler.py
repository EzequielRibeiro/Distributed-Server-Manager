...
def _settings(config: dict[str, Any]) -> tuple[int, int, int]:
    ...
    configured = max(1, int(config.get("reconcile_failure_threshold", DEFAULT_FAILURE_THRESHOLD)))
    threshold = min(configured, limits.max_reconcile_retries)
...
def _event(event_type: str, record: dict[str, Any], data: dict[str, Any] | None = None) -> None:
    emit_runtime_event(
        Path(instance_runtime.STATE_DIR), event_type,
        instance_id=str(record["instance_id"]), agent_id=str(record["agent_id"]), data=dict(data or {}),
    )
...
def _failure(config: dict[str, Any], record: dict[str, Any], exc: Exception, *, drift: str | None = None) -> dict[str, Any]:
    retries = int(record.get("reconcile_retry_count") or 0) + 1
    ...
    increment("reconcile_failed")
    if status == "degraded":
        increment("instance_degraded")
    _event("INSTANCE_DEGRADED" if status == "degraded" else "INSTANCE_RECONCILE_FAILED", updated,
           {"retry_count": retries, "next_retry_at": next_retry, "error": error, "drift": drift})
    return {"instance_id": record["instance_id"], "status": status, "retry_count": retries,
            "next_retry_at": next_retry, "error": error, "drift": drift}


def _reconcile_locked(config: dict[str, Any], record: dict[str, Any], normalized: dict[str, Any]) -> dict[str, Any]:
    drift: str | None = None
    recovered = False
    ...
    try:
        ...
        updated = _save(latest, status="healthy", observed_state=observed, drift=None, retry_count=0,
                        next_retry_at=None, error=None, recovery_action=recovery_action)
        if recovered:
...
def reconcile_instance(config: dict[str, Any], instance_id: str, *, force: bool = False) -> dict[str, Any]:
    ...
    try:
        with runtime_operation(config, normalized["instance_id"], "reconcile", lock_timeout_seconds=limits.lock_timeout_seconds):
            result = _reconcile_locked(config, record, normalized)
    except Exception as exc:
        latest = instance_runtime.get_instance(normalized["instance_id"]) or record
        result = _failure(config, latest, exc, drift="operation_conflict")
    observe_duration("reconcile", int((time.monotonic() - started) * 1000))
    return result
...
def reconciliation_inventory(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in instance_runtime.list_instances(config):
        ...
        if not record:
            continue
        values.append({"instance_id": instance_id, "desired_state": record.get("desired_state"),
                       ...
                       "last_error": record.get("reconcile_last_error"), "drift": record.get("reconcile_drift")})
    return values
