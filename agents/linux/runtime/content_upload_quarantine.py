#!/usr/bin/env python3
"""Quarantine contract for customer-supplied content archives on Linux Agents."""
from __future__ import annotations
import os,stat,tarfile,zipfile
from pathlib import Path
from typing import Iterable
GAME_DATA_ROOT=Path(os.environ.get("CAPIVARA_GAME_DATA_ROOT",os.environ.get("CAPIVARA_AGENT_GAME_DATA_ROOT","/var/lib/capivara-agent/game-data"))).resolve()
QUARANTINE_ROOT=(GAME_DATA_ROOT/"quarantine").resolve()
_MAX_ENTRIES=100000;_MAX_EXPANDED=128*1024*1024*1024
_ALLOWED_SUFFIXES=(".zip",".tar",".tar.gz",".tgz",".jar")
...
def _filename(value):
 ...
 if not name or name in {".",".."} or name!=str(value or ""):raise ValueError("invalid content upload filename")
 if not name.lower().endswith(_ALLOWED_SUFFIXES):raise ValueError("unsupported content archive")
 return name

def quarantine_destination(instance_id,transfer_id,filename):
 root=QUARANTINE_ROOT;candidate=(root/_token(instance_id)/_token(transfer_id)/_filename(filename)).resolve();candidate.relative_to(root);return candidate
...
def validate_quarantine_archive(path:Path)->dict:
 ...
 if zipfile.is_zipfile(archive):
  ...
  return {"archive_type":"jar" if archive.name.lower().endswith(".jar") else "zip","entries":len(entries)}
