...
def inspect_game_data(root: Path, selection: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if executable:
        ...
        try: candidate.resolve().relative_to(root); executable_present = candidate.is_file()
        except ValueError: executable_present = False
    health = "ok" if files and executable_present and not truncated else ("degraded" if files else "empty")
    return {"health": health, "exists": True, "files": min(files, MAX_INVENTORY_FILES), "bytes": total, "tree_digest": digest.hexdigest(), "truncated": truncated, "executable_present": executable_present}
