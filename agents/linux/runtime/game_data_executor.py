...
def _target_for(selection:dict[str,Any])->Path:
 game=_safe_name(selection.get("game"),"game");declared=Path(str(selection.get("install_dir") or "serverfiles"));leaf=_safe_name(declared.name if declared.name not in {"",".","/"} else "serverfiles","install target")
 target=(GAME_DATA_ROOT/game/leaf).resolve();target.relative_to(GAME_DATA_ROOT);return target
...
def _run_steam(selection:dict[str,Any],target:Path)->None:
 ...
 cp=subprocess.run([_steamcmd(),"+force_install_dir",str(target),"+login",login,"+app_update",app_id,"validate","+quit"],stdin=subprocess.DEVNULL,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=7200,check=False,env={**os.environ,"HOME":os.environ.get("HOME","/var/lib/capivara-agent")})
 output=cp.stdout or "";print(output,end="" if output.endswith("\n") else "\n",flush=True)
 if cp.returncode!=0:
...
def _execute(command:dict[str,Any])->dict[str,Any]:
 ...
 target=_target_for(selection);provider=str(selection.get("provider") or "").strip().lower()
 if action in {"install","update","repair"}:_install(selection,target,provider)
 ...
 if action=="verify" and integrity.get("health")!="ok":raise RuntimeError(f"game-data integrity check failed: {integrity.get('health')}")
 return {"provider":provider,"game":selection.get("game"),"version":selection.get("version"),"target_path":str(target),"integrity":integrity}
...
def main()->int:
 ...
 if action in {"install","update","verify","repair"}:
