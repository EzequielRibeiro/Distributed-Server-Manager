...
def _steamcmd() -> str:
    for candidate in (
        ...
        "/usr/games/steamcmd",
        "/opt/dsm/tools/steamcmd/steamcmd.sh",
    ):
...
def _run_steam(selection: dict[str, Any], target: Path) -> None:
    ...
    if completed.returncode != 0:
        if "password" in output.lower() or "steam guard" in output.lower() or "two-factor" in output.lower():
...
def _run_http(selection: dict[str, Any], target: Path) -> None:
    ...
    archive_type = str((selection.get("archive") or {}).get("type") if isinstance(selection.get("archive"), dict) else "")
    archive_type = archive_type or str(install.get("archive_type") or "")
    target.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="capivara-game-data-") as temporary:
        ...
        if archive_type or zipfile.is_zipfile(artifact) or tarfile.is_tarfile(artifact):
            ...
            filename = str(asset.get("name") or install.get("asset") or selection.get("executable") or "artifact")
            filename = _safe_name(Path(filename).name, "artifact filename")
