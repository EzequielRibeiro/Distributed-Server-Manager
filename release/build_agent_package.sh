...
mkdir -p "${PACKAGE_ROOT}/agent/common" "${PACKAGE_ROOT}/agent/runtime/adapters" "${PACKAGE_ROOT}/agent/runtime/materializers" \
  "${PACKAGE_ROOT}/agent/runtime/profiles" "${PACKAGE_ROOT}/agent/privileged" "${PACKAGE_ROOT}/agent/updater" \
...
for file in agent.py capabilities.py configuration_client.py network_inventory.py update_client.py update_state.py local_cli.py cap_dispatch.py game_data_client.py game_data_executor.py game_data_state.py instance_runtime.py runtime_spec.py runtime_events.py runtime_materialization.py runtime_reconciler.py runtime_lock.py runtime_limits.py runtime_operations.py runtime_health.py runtime_metrics.py game_runtime.py provisioning_contract.py provisioning_state.py provisioning_client.py provisioning_executor.py privileged_materialization.py; do
  copy "agents/linux/runtime/${file}" "agent/runtime/${file}"
done
...
for file in __init__.py base.py registry.py systemd.py; do
  copy "agents/linux/runtime/adapters/${file}" "agent/runtime/adapters/${file}"
  copy "agents/linux/runtime/materializers/${file}" "agent/runtime/materializers/${file}"
done
...
required = [p.relative_to(root).as_posix() for p in root.rglob('*') if p.is_file() and p.name != 'manifest.json']
...
manifest = {"schema_version": 1, "kind": "CapivaraAgentPackage", "platform": "linux", "version": version,
            "git_commit": commit, "channel": channel, "required_files": required, "files": files}
