...
def install_deleted_backup_vault_http(legacy,authenticate):
 ...
 def actor(self):
  # This is a Customer-only API: never select the Controller cookie first.
  # A browser may have valid Controller and Customer sessions simultaneously.
  ...
  try:
   ...
   if final_backup:
    try:
     ...
     return self.send_json(403,{"error":"backup_permission_required","message":"O backup final exige permissões para criar e baixar backups. Solicite acesso ou escolha a exclusão sem backup, ciente de que ela é irreversível."})
    item,idempotent=vault().start(instance_id,requested_by=requested_by,retention_hours=168);return self.send_json(202,{"mode":"backup_then_remove","vault":public_item(item),"idempotent":idempotent})
