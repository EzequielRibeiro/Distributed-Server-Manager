...
LIST_PATH="/api/customer/deleted-backups";DOWNLOAD_PATH=LIST_PATH+"/download";DELETE_PATH="/api/customer/instance/delete";STATUS_PATH=DELETE_PATH+"/status"
...
def install_deleted_backup_vault_http(legacy,authenticate):
 ...
 def customer(self):
  ...
  try:customer_id=int(user.get("scope_id"))
 ...
 def post(self):
  parsed=urlparse(self.path);path=parsed.path
  if path!=DELETE_PATH:return previous_post(self)
  resolved=customer(self)
  if resolved is None:return
  user,customer_id=resolved
  try:
   body=self.read_json_body();instance_id=str(body.get("instance_id") or "").strip();context=workspace().require(user,instance_id,"instance.delete")
   expected=str(context.get("name") or context.get("id") or "").strip();confirmation=str(body.get("confirmation") or "").strip()
   ...
   if final_backup:
    workspace().require(user,instance_id,"backup.create");workspace().require(user,instance_id,"backup.download");item,idempotent=vault().start(instance_id,requested_by=requested_by,retention_hours=168)
    return self.send_json(202,{"mode":"backup_then_remove","vault":public_item(item),"idempotent":idempotent})
   command=AgentInstanceRuntimeRepository(backend()).enqueue(agent_id=str(context.get("agent_id") or ""),instance_id=instance_id,action="remove",requested_by=requested_by)
   return self.send_json(202,{"mode":"remove","command_id":command.get("command_id"),"status":command.get("status")})
  except PermissionError:return self.send_json(403,{"error":"forbidden"})
...
__all__=["LIST_PATH","DOWNLOAD_PATH","DELETE_PATH","STATUS_PATH","install_deleted_backup_vault_http"]
