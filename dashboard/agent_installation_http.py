#!/usr/bin/env python3
"""Transport-neutral HTTP dispatchers for Phase 14 Agent installation UI."""

from __future__ import annotations

from typing import Any

from agent_installation_api import (
    agent_installation_status_for_user,
...
def _error(exc: Exception) -> tuple[int, dict[str, Any]]:
    ...
    if isinstance(exc, AgentReleaseError):
...
def dispatch_agent_installation_post(path: str, payload, *, user, backend):
    if path != AGENT_INSTALLATIONS_PATH:
        return None
    try:
...
def dispatch_agent_installation_get(
    path: str,
    *,
    user,
    backend,
    ...
    try:
        ...
        if path == AGENT_RELEASES_PATH:
            ...
            return 200, {
                ...
                "releases": releases,
                "recommended": releases[0]["tag"] if releases else None,
            }
        if path != AGENT_INSTALLATION_STATUS_PATH:
            return None
        return 200, agent_installation_status_for_user(
            user,
            backend,
