...
def runtime_definition(root:Path,game:str,runtime_id:str)->dict[str,Any]:
 ...
 if not definitions.is_dir():raise ValueError("game is not available in the catalog")
 for path in definitions.glob("*.json"):
  try:definition=json.loads(path.read_text(encoding="utf-8"))
  except (OSError,json.JSONDecodeError):continue
  if definition.get("id")==runtime_id:return definition
...
def install_customer_instance_creation(legacy)->None:
 ...
 def create_customer_instance(user,payload,root=None,database_path=None):
  ...
  for value,label in ((runtime_id,"runtime_id"),(edition,"edition"),(version,"version"),(build,"build")):
  ...
  contract=legacy.customer_contract_for_runtime(user,game,runtime_id,contract_id,database_path=database_path);contract_id=str(contract.get("id") or "").strip()
