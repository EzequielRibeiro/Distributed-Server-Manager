...
def dispatch_enroll(payload: dict[str, Any] | None, *, backend) -> tuple[int, dict[str, Any]]:
    ...
    tracking_bound = True
    try:
        bind_installation_after_enrollment(backend, pairing_token=str((payload or {}).get("pairing_token", "")), agent_id=str(result["agent_id"]))
    except Exception:
        tracking_bound = False
    identity = dict(result.get("identity") or {})
    return 201, {
        "agent_id": result["agent_id"], "node_id": result["node_id"], "controller_id": result["controller_id"],
        "status": result["status"], "credential_id": identity["credential_id"],
        "credential_secret": identity["credential_secret"], "credential_type": identity["credential_type"],
        "fingerprint": identity["fingerprint"], "pairing_token_consumed": bool(result.get("pairing_token_consumed")),
...
def _attach_update_state(result: dict[str, Any], body: dict[str, Any], *, agent_id: str, backend) -> None:
    try:
        updates = AgentUpdateRepository(backend); updates.initialize()
        update_state = updates.reconcile_after_heartbeat(agent_id, body.get("capivara_version"), result.get("health_status", "online"))
        ...
        if update_result and str(update_result.get("status", "")).lower() == "failed":
            update_state = updates.mark_failed(agent_id, str(update_result.get("error", "update failed"))[:2000]); command = None
        else:
...
def _attach_game_data_state(result: dict[str, Any], body: dict[str, Any], *, agent_id: str, backend) -> None:
    try:
        jobs = AgentGameDataRepository(backend); jobs.initialize()
        ...
        command = jobs.command_for_agent(agent_id); command_state = None
        if command:
            command_state = jobs.mark_delivered(str(command["job_id"])); result["game_data_command"] = command
        result["game_data_state"] = reported_state or command_state or {"status": "idle"}
    except Exception:
        result["game_data_state"] = {"status": "unavailable"}


def _attach_instance_provisioning_state(result: dict[str, Any], body: dict[str, Any], *, agent_id: str, backend) -> None:
    """Best-effort provisioning transport; failures must not suppress liveness."""
    try:
        jobs = AgentInstanceProvisioningRepository(backend); jobs.initialize()
        reported = body.get("instance_provisioning_result") if isinstance(body.get("instance_provisioning_result"), dict) else None
        reported_state = jobs.apply_result(agent_id, reported) if reported else None
        command = jobs.command_for_agent(agent_id); command_state = None
        if command:
            command_state = jobs.mark_delivered(str(command["job_id"])); result["instance_provisioning_command"] = command
        result["instance_provisioning_state"] = reported_state or command_state or {"status": "idle"}
    except Exception:
        result["instance_provisioning_state"] = {"status": "unavailable"}


def _attach_instance_state(result: dict[str, Any], body: dict[str, Any], *, agent_id: str, backend) -> None:
    try:
        commands = AgentInstanceRuntimeRepository(backend); commands.initialize()
        ...
        command = commands.command_for_agent(agent_id); command_state = None
        if command:
            command_state = commands.mark_delivered(str(command["command_id"])); result["instance_command"] = command
...
def dispatch_heartbeat(payload: dict[str, Any] | None, *, headers, backend) -> tuple[int, dict[str, Any]]:
    ...
    try:
        identity = AgentPairingRepository(backend).authenticate(
            credential_id=credential_id, credential_secret=credential_secret, fingerprint=fingerprint
        )
        ...
        _attach_instance_provisioning_state(result, body, agent_id=identity["agent_id"], backend=backend)
