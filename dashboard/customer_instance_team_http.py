...
TEAM_PATH="/api/customer/instance/workspace/team"
TEAM_INVITE_PATH=TEAM_PATH+"/invite"
TEAM_GRANTS_PATH=TEAM_PATH+"/grants"
...
def install_customer_instance_team(legacy,authenticate):
    ...
    def require_owner(actor,instance_id):
        if actor is None:raise PermissionError("authentication required")
        if str(actor.get("role") or "").lower() in {"admin","controller"}:return
        if str(actor.get("role") or "").lower()!="customer":raise PermissionError("customer access required")
        ...
        with backend().connect() as c:
            ...
            try:row=s.execute(f"SELECT account_role FROM customer_account_members WHERE customer_id={ph} AND username={ph}",(int(context["customer_id"]),str(actor.get("username") or "").lower())).fetchone()
            finally:s.close()
    ...
    def get(self):
        ...
        actor=user(self);instance_id=iid(parsed)
        try:
            require_owner(actor,instance_id);repo=InstanceTeamRepository(backend());self.send_json(200,{"members":repo.members(instance_id),"available_permissions":sorted(INSTANCE_PERMISSIONS)})
        except Exception as exc:error(self,exc)
    def post(self):
        parsed=urlparse(self.path)
        if parsed.path!=TEAM_INVITE_PATH:return previous_post(self)
        actor=user(self)
        try:
            body=self.read_json_body();instance_id=iid(parsed,body);require_owner(actor,instance_id);created=InstanceTeamRepository(backend()).invite(instance_id=instance_id,email=body.get("email"),grants=body.get("grants") or {},invited_by=str(actor.get("username") or ""));self.send_json(201,created)
        except Exception as exc:error(self,exc)
    def patch(self):
        parsed=urlparse(self.path)
        ...
        actor=user(self)
        try:
            body=self.read_json_body();instance_id=iid(parsed,body);require_owner(actor,instance_id);username=str(body.get("username") or "").strip().lower();grants=body.get("grants") or {};saved=InstanceWorkspaceRepository(backend()).set_permission_grants(username,instance_id,grants);self.send_json(200,{"username":username,"grants":saved,"permissions":sorted(InstanceWorkspaceRepository(backend()).effective_permissions_for(username,instance_id))})
        except Exception as exc:error(self,exc)
    legacy.DashboardHandler.do_GET=get;legacy.DashboardHandler.do_POST=post;legacy.DashboardHandler.do_PATCH=patch

__all__=["TEAM_PATH","TEAM_INVITE_PATH","TEAM_GRANTS_PATH","install_customer_instance_team"]
