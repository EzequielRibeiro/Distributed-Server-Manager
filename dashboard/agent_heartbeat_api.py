...
def _store_agent_metadata(agent_id: str, body: dict[str, Any], *, backend) -> None:
    logs = body.get("agent_logs"); safe_logs = None
    if isinstance(logs, list): safe_logs = [str(line).replace("\x00", "")[:2000] for line in logs[-200:] if isinstance(line, str)]
    telemetry = _telemetry_from_heartbeat(body)
    instance_telemetry = body.get("instance_telemetry") if isinstance(body.get("instance_telemetry"), list) else None
    console_state = body.get("instance_console_state") if isinstance(body.get("instance_console_state"), list) else None
    ...
    with backend.transaction() as connection:
        ...
        try:
            row = session.execute(f"SELECT metadata_json FROM agents WHERE id={ph}",(agent_id,)).fetchone(); raw_metadata = row["metadata_json"] if row else None
            ...
            if instance_telemetry is not None: metadata["instance_telemetry"] = [dict(item) for item in instance_telemetry[-500:] if isinstance(item, dict)]
            if console_state is not None:
                ...
                for item in console_state[-200:]:
                    ...
                    safe_console.append(value)
                metadata["instance_console_state"]=safe_console
...
def _store_instance_telemetry(agent_id: str, body: dict[str, Any], *, backend) -> int:
    samples=body.get("instance_telemetry")
    if not isinstance(samples,list):return 0
    repository=InstanceWorkspaceRepository(backend);repository.initialize();accepted=0
    for sample in samples[:500]:
        ...
        try:
            ...
            repository.record_telemetry(instance_id,sample);accepted+=1
        except (KeyError,ValueError,PermissionError):continue
    return accepted


def _console_exchange(agent_id: str, body: dict[str, Any], *, backend) -> tuple[dict[str,Any]|None,dict[str,Any]|None]:
    repository=InstanceWorkspaceRepository(backend);repository.initialize();state=None
    reported=body.get("console_result")
    if isinstance(reported,dict):state=repository.apply_console_result(agent_id,reported)
    command=repository.command_for_agent(agent_id)
    if command is not None:repository.mark_console_delivered(str(command["command_id"]))
...
def record_agent_heartbeat(authenticated_agent_id: str, payload: dict[str, Any] | None, *, backend) -> dict[str, Any]:
    ...
    telemetry_count=_store_instance_telemetry(agent_id,body,backend=backend)
