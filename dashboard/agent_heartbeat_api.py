    ...
    with backend.transaction() as connection:
        ...
        try:
            ...
            session.execute(
                ...
                (json.dumps(metadata, separators=(",", ":")), agent_id),
            )
        finally:
            session.close()
    ...
    if isinstance(runtime, dict):
        ...
        for name, stats in (runtime.get("durations_ms") or {}).items():
            ...
            for field in ("count", "total", "max"):
                ...
                if isinstance(value, (int, float)) and not isinstance(value, bool):
                    result.append({"metric_name": f"capivara.runtime.duration.{_metric_token(name)}.{field}", "metric_type": "counter" if field in {"count", "total"} else "gauge", "value": value, "unit": "1" if field == "count" else "milliseconds", "scope_type": "agent"})
...
def record_agent_heartbeat(authenticated_agent_id: str, payload: dict[str, Any] | None, *, backend) -> dict[str, Any]:
    ...
    if any(inventory_fields[n] is not None for n in ("hostname", "os_name", "architecture", "capivara_version", "address", "fingerprint", "capabilities", "cpu", "ram_total_bytes", "storage", "network")):
    ...
    if isinstance(reconciliation, list):
        repo = AgentInstanceReconciliationRepository(backend)
        repo.initialize()
        repo.apply_inventory(agent_id, reconciliation)
    runtime_health = body.get("instance_runtime_health")
    if isinstance(runtime_health, list):
        health_repo = AgentInstanceRuntimeHealthRepository(backend)
        health_repo.initialize()
        health_repo.apply_inventory(agent_id, runtime_health)
    ...
    contents = ContentRepository(backend)
    contents.initialize()
    reported_content = body.get("content_state")
    if isinstance(reported_content, list):
        contents.record_agent_state(agent_id, reported_content)
    desired_content = contents.desired_for_agent(agent_id)

    backups = BackupRepository(backend)
    backups.initialize()
    reported_backups = body.get("backup_state")
    if isinstance(reported_backups, list):
        backups.record_agent_state(agent_id, reported_backups)
    backup_commands = backups.commands_for_agent(agent_id)

    automations = AutomationRepository(backend)
    automations.initialize()
    reported_broadcasts = body.get("broadcast_state")
    if isinstance(reported_broadcasts, list):
        automations.record_broadcast_state(agent_id, reported_broadcasts)
    ...
    return {
        ...
        "content_commands": desired_content,
        "content_count": len(desired_content),
        "backup_commands": backup_commands,
        "backup_count": len(backup_commands),
        "broadcast_commands": broadcast_commands,
        "broadcast_count": len(broadcast_commands),
    }
