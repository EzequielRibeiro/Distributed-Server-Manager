...
def _artifact_exchange(agent_id,body,*,backend,root=None,deliver_commands=True):
 ...
 return (repo.command_for_agent(agent_id) if deliver_commands else None),state
def _native_restart_exchange(agent_id,body,*,backend,deliver_commands=True):
 repo=NativeRestartRepository(backend);repo.initialize();state=None
 reported=body.get("native_restart_result")
 if not isinstance(reported,dict):reported=body.get("dayz_native_restart_result")
 if isinstance(reported,dict):state=repo.apply_result(agent_id,reported)
 cmd=repo.command_for_agent(agent_id) if deliver_commands else None
 if cmd is not None:repo.mark_delivered(str(cmd["command_id"]))
 return cmd,state

def _dayz_operation_exchange(agent_id,body,*,backend,deliver_commands=True):
 repo=DayZManagementRepository(backend);repo.initialize();state=None;reported=body.get("dayz_operation_result")
 if isinstance(reported,dict):state=repo.apply_result(agent_id,reported)
 cmd=repo.command_for_agent(agent_id) if deliver_commands else None
 if cmd is not None:repo.mark_delivered(str(cmd["operation_id"]))
 return cmd,state
...
def _storage_pool_migration_exchange(agent_id,body,*,backend):
 ...
 return cmd,state
def _yarax_admin_exchange(agent_id,body,*,backend,deliver_commands=True):
 repo=YaraXAdminOperationRepository(backend);repo.initialize();state=None;reported=body.get("yarax_admin_result")
 if isinstance(reported,dict):state=repo.apply_result(agent_id,reported)
 return (repo.command_for_agent(agent_id) if deliver_commands else None),state
...
def record_agent_heartbeat(authenticated_agent_id,payload,*,backend,root=None):
 ...
 desired_content=contents.desired_for_agent(agent_id);updates=ServerUpdateRepository(backend);updates.initialize();reported_updates=body.get("content_update_inventory");update_inventory_result={"accepted":0,"rejected":0}
 if isinstance(reported_updates,dict):update_inventory_result=updates.record_content_update_inventory(agent_id,reported_updates)
 ...
 deliver_commands=not bool(body.get("result_flush"));yarax_admin_command,yarax_admin_state=_yarax_admin_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);console_command,console_state=_console_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);file_command,file_state=_file_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);resource_command,resource_state=_resource_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);artifact_command,artifact_state=_artifact_exchange(agent_id,body,backend=backend,root=root,deliver_commands=deliver_commands);native_restart_command,native_restart_state=_native_restart_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);dayz_operation_command,dayz_operation_state=_dayz_operation_exchange(agent_id,body,backend=backend,deliver_commands=deliver_commands);storage_pool_migration_command,storage_pool_migration_state=_storage_pool_migration_exchange(agent_id,body,backend=backend);uninstall_command,uninstall_state=_uninstall_exchange(agent_id,body,backend=backend);last_seen=repository.heartbeat(agent_id)
 return {"agent_id":agent_id,"health_status":"online","last_seen":last_seen,"accepted_event_ids":event_result["accepted_event_ids"],"events_accepted":event_result["accepted"],"events_created":event_result["created"],"events_rejected":event_result["rejected"],"metrics_accepted":metric_result["accepted"],"metrics_created":metric_result["created"],"metrics_rejected":metric_result["rejected"],"instance_telemetry_accepted":telemetry_count,"configuration_commands":desired_configuration,"configuration_count":len(desired_configuration),"content_commands":desired_content,"content_count":len(desired_content),"content_updates_accepted":update_inventory_result["accepted"],"content_updates_rejected":update_inventory_result["rejected"],"backup_commands":backup_commands,"backup_count":len(backup_commands),"broadcast_commands":broadcast_commands,"broadcast_count":len(broadcast_commands),"console_command":console_command,"console_state":console_state,"file_command":file_command,"file_state":file_state,"resource_command":resource_command,"resource_state":resource_state,"artifact_command":artifact_command,"artifact_state":artifact_state,"native_restart_command":native_restart_command,"native_restart_state":native_restart_state,"dayz_native_restart_command":native_restart_command if isinstance(native_restart_command,dict) and str(native_restart_command.get("game_id") or "").lower()=="dayz" else None,"dayz_native_restart_state":native_restart_state if isinstance(native_restart_state,dict) and str(native_restart_state.get("game_id") or "").lower()=="dayz" else None,"dayz_operation_command":dayz_operation_command,"dayz_operation_state":dayz_operation_state,"storage_pool_migration_command":storage_pool_migration_command,"storage_pool_migration_state":storage_pool_migration_state,"uninstall_command":uninstall_command,"uninstall_state":uninstall_state,"yarax_admin_command":yarax_admin_command,"yarax_admin_state":yarax_admin_state}
