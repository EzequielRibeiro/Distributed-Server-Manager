...
async function loadLogs() {
  ...
  if (!target) return;
  const params = new URLSearchParams({source:"controller", limit:"500"});
  const response = await get(`/log-viewer?${params.toString()}`);
  const data = response?.data || response || {};
  const logs = Array.isArray(data.logs) ? data.logs : [];
  text("logs-context", "Controller");
  target.replaceChildren();
  if (!logs.length) {
    const empty = document.createElement("div");
    empty.className = "cap-empty-state";
    empty.textContent = data.message || "Nenhum log do Controller disponível.";
