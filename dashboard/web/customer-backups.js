(function () {
  ...
  function instanceUrl(item) {
    return "/customer-instance.html?" + new URLSearchParams({server: item.server || "", game: item.game || "", instance: item.instance || item.id || ""});
  }
  ...
  async function loadCard(item) {
    ...
    const status = card.querySelector(".integration-status"); const list = card.querySelector(".integration-list");
    try {
      ...
      const completed = (jobs.jobs || []).filter(job => job.action === "create" && job.status === "completed").slice(0, 1)[0];
      status.textContent = completed ? "Protegido" : "Sem backup"; status.classList.add(completed ? "online" : "warning");
      ...
      status.textContent = "Indisponível"; status.classList.add("warning");
  ...
  async function load() {
    ...
    try {
      ...
      if (!instances.length) { grid.innerHTML = '<div class="integration-empty">Nenhum servidor criado.</div>'; return; }
      const cards = await Promise.all(instances.map(loadCard)); grid.replaceChildren(...cards);
    } catch (error) { grid.innerHTML = `<div class="integration-notice">${error.message}</div>`; }
  }
  load();
})();
