...
(() => {
    ...
    function updateUi() {
        ...
        el("generate-agent-install").textContent = ssh ? `Instalar Agent ${platform === "windows" ? "Windows" : "Linux"} via SSH` : winrm ? "Instalar Agent Windows via WinRM" : "Gerar instalação";
        if (ssh && !el("agent-controller-url").value) el("agent-controller-url").value = window.location.origin;
        if (winrm && !el("agent-winrm-controller-url").value) el("agent-winrm-controller-url").value = window.location.origin;
        const note = el("agent-manual-connectivity-note");
        if (note) note.hidden = ssh || winrm;
        renderReview();
    }
    ...
    function renderReview() {
        ...
        review.innerHTML = `<dl class="cap-agent-review-grid"><div><dt>Plataforma</dt><dd>${platform === "windows" ? "Windows" : "Linux"}</dd></div><div><dt>Método</dt><dd>${methodLabel}</dd></div><div><dt>Release</dt><dd>${method === "local" ? "Pacote local" : textValue("agent-release-tag")}</dd></div><div><dt>Host remoto</dt><dd>${remoteHost}</dd></div><div><dt>Controller alcançável</dt><dd>${remoteUrl}</dd></div><div><dt>Região</dt><dd>${textValue("agent-install-region")}</dd></div><div><dt>Datacenter</dt><dd>${textValue("agent-install-datacenter")}</dd></div><div><dt>Nome administrativo</dt><dd>${textValue("agent-preconfig-name")}</dd></div><div><dt>Faixa de portas</dt><dd>${textValue("agent-preconfig-port-start")}–${textValue("agent-preconfig-port-end")} / ${textValue("agent-preconfig-protocol")}</dd></div><div><dt>Endpoint público</dt><dd>${textValue("agent-public-hostname",textValue("agent-public-ipv4","Não configurado"))}</dd></div></dl>`;
    }
    ...
    async function generate(event) {
        ...
        const payload = {
            ...
            controller_url: method === "ssh" ? el("agent-controller-url").value : method === "winrm" ? el("agent-winrm-controller-url").value : window.location.origin,
    ...
    async function init() {
        ...
        populateTopology();
        ensureReleaseUi();
        document.querySelectorAll('input[name="agent-method"]').forEach(x => x.addEventListener("change",async () => { updateUi(); await loadReleases(); }));
