...
function runtimeState(){return String(overview?.runtime?.state||overview?.instance?.status||$("state")?.textContent||"").trim().toLowerCase()}
function syncRuntimeState(){if(!overview)return;const state=runtimeState(),node=$("state"),running=["running","online"].includes(state);if(node){node.textContent=state||"desconhecido";node.className=`state ${running?"ok":state==="failed"?"warn":""}`}}
...
function applyControls(){if(!overview)return;const p=permissions(),pr=!!overview.provision,state=runtimeState(),running=["running","online"].includes(state),stopped=["stopped","offline"].includes(state),startable=stopped||state==="failed",busy=["starting","stopping","restarting"].includes(state),pending=!!lifecyclePending,start=$("start"),stop=$("stop"),restart=$("restart");if(start)start.disabled=pending||!p.has("instance.start")||pr||!startable||busy;if(stop)stop.disabled=pending||!p.has("instance.stop")||pr||!running||busy;if(restart)restart.disabled=pending||!p.has("instance.restart")||pr||!running||busy;setConsoleInteractive(!!overview?.console?.supported&&p.has("console.execute"))}
async function refreshOverview(){if(!iid||overviewBusy)return;overviewBusy=true;try{overview=await get(`${api}?instance_id=${encodeURIComponent(iid)}`);syncRuntimeState();applyControls()}catch{}finally{overviewBusy=false}}
