...
function applyControls(){if(!overview)return;const p=permissions(),pr=!!overview.provision,state=runtimeState(),running=["running","online"].includes(state),stopped=["stopped","offline"].includes(state),busy=["starting","stopping","restarting"].includes(state),pending=!!lifecyclePending,start=$("start"),stop=$("stop"),restart=$("restart");if(start)start.disabled=pending||!p.has("instance.start")||pr||running||busy;if(stop)stop.disabled=pending||!p.has("instance.stop")||pr||stopped||busy;if(restart)restart.disabled=pending||!p.has("instance.restart")||pr||!running||busy;const consoleWrap=$("console-command-wrap"),interactive=!!overview?.console?.supported&&p.has("console.execute");if(consoleWrap)consoleWrap.classList.toggle("hidden",!interactive)}
