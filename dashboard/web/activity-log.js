(function () {
  ...
  const roleLabels = {admin: "Administrador", controller: "Controlador", operator: "Operador", customer: "Cliente"};

  async function request(path) {
    const response = await fetch(path, {headers: controllerHeaders(), credentials: "same-origin", cache: "no-store"});
  ...
  function resetActors(message) {
    ...
    actorState.offset = 0; actorState.hasMore = false; actorState.showAll = false; actorState.query = "";
  ...
  function actorLabel(actor) {
    ...
    if (actor.customer_code) parts.push(String(actor.customer_code)); else if (actor.document_number) parts.push(String(actor.document_number));
  ...
  function appendActors(actors, append) {
    ...
    (actors || []).forEach(actor => {
      ...
      const option = new Option(actorLabel(actor), id); option.dataset.role = String(actor.actor_role || "");
      select.appendChild(option); existing.add(id);
    });
  ...
  async function loadActors({ append = false, showAll = false } = {}) {
    ...
    if (!showAll && !term) { resetActors("Informe nome, e-mail, documento, login ou ID, ou use “Mostrar lista completa”."); return; }
    const offset = append ? actorState.offset : 0;
    const query = new URLSearchParams(); const role = byId("activity-role").value;
    if (role) query.set("role", role); if (term) query.set("q", term); if (showAll) query.set("show_all", "true");
    ...
    actorState.offset = Number(data.next_offset || 0); actorState.hasMore = Boolean(data.has_more); actorState.showAll = showAll; actorState.query = term;
    byId("activity-more-users").hidden = !actorState.hasMore;
    const total = Number(data.total || 0); const shown = Math.min(actorState.offset, total);
  ...
  function params() {
    ...
    [["actor_role",byId("activity-role").value],["actor_id",byId("activity-user").value],["category",byId("activity-category").value],["action",byId("activity-name").value],["result",byId("activity-result").value],["start_at",isoLocal(byId("activity-start").value)],["end_at",isoLocal(byId("activity-end").value)],["limit",byId("activity-limit").value]].forEach(([key,value])=>{if(value)query.set(key,value);});
  ...
  function render(rows) {
    ...
    rows.forEach(item => { const row=document.createElement("tr"); [formatDate(item.occurred_at),item.actor_name||item.actor_id||"—",item.summary||"—",item.category||"—",item.result||"—",item.target_name||item.target_id||"—"].forEach(value=>{const td=document.createElement("td");td.textContent=value;row.appendChild(td);}); body.appendChild(row); });
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    try {
      await loadShell(); await loadOptions();
      byId("activity-search").onclick=()=>search().catch(showError); byId("activity-clear").onclick=clearFilters;
      byId("activity-find-user").onclick=()=>loadActors({showAll:false}).catch(showError); byId("activity-show-users").onclick=()=>loadActors({showAll:true}).catch(showError); byId("activity-more-users").onclick=()=>loadActors({append:true,showAll:actorState.showAll}).catch(showError);
