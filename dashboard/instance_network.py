...
from core.network.port_inspector import LocalPortInspector, PortInspectionError
...
_TRUSTED_NETWORK_SOURCES = {"ss", "netstat"}
...
def occupied_ports_for_agent(
    ...
    if not local_node:
        raise PortInspectionError(
            "DSM_LOCAL_NODE_ID is required for local Agent port inspection"
        )
    if local_node != node_id:
        raise PortInspectionError(
            "remote Agent port inspection requires a backend-aware provider"
        )
    return LocalPortInspector().occupied(protocol, start_port, end_port)
...
def _remote_inventory_response(
    ...
    A remote inventory is accepted only when the Agent is online and the
    requested protocol was collected successfully. Collector failures are
    explicitly distinguished from a valid empty socket set so allocation
    cannot silently assume that an unreachable inspector means "no ports".
    """
    try:
    ...
    if source not in _TRUSTED_NETWORK_SOURCES:
        raise PortInspectionError(
            "remote Agent network inventory source is not trusted"
        )

    protocol = request.protocol.lower()
    key = {"tcp": "tcp_listen", "udp": "udp_listen"}.get(protocol)
    if key is None:
        raise PortInspectionError(
            f"unsupported port inspection protocol: {request.protocol}"
        )

    if network.get(f"{protocol}_complete") is not True:
        raise PortInspectionError(
...
def _format_template(template: str, ports: Mapping[str, int]) -> str:
    try:
        return template.format(**ports)
    except KeyError as exc:
        raise ValueError(
            "network application references unknown port: "
            f"{exc.args[0]}"
        ) from exc


def _property_path(instance_path: Path, relative: str) -> Path:
    ...
    if target.is_symlink():
        raise ValueError(
            "network property file cannot be a symbolic link"
        )
    return target


def _write_property(path: Path, key: str, value: str) -> None:
    text = (
