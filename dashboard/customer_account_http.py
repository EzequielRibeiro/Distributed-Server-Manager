...
from customer_account_api import (
    member_capabilities,
...
def _username_for_email(session: AlertSession, dialect, email: str) -> str:
    ...
    for attempt in range(101):
        candidate = (
            seed
            if attempt == 0
            else f"{seed[:22].rstrip('-._')}-{uuid.uuid5(uuid.NAMESPACE_URL, email + str(attempt)).hex[:8]}"
        )
        if session.execute(
            f"SELECT 1 FROM dashboard_users WHERE username={ph}", (candidate,)
        ).fetchone() is None:
            return candidate
...
def _default_controller(session: AlertSession) -> str:
    ...
    if configured:
        row = session.execute(
            f"SELECT id FROM controllers WHERE id={ph} AND status='active'",
            (configured,),
        ).fetchone()
        if row is None:
            raise ValueError("customer registration controller is unavailable")
        return configured
    rows = session.execute(
        "SELECT id FROM controllers WHERE status='active' ORDER BY id LIMIT 2"
    ).fetchall()
    if len(rows) != 1:
        raise ValueError("customer registration requires a configured controller")
    return str(rows[0]["id"])


def register_customer(payload: dict[str, Any], backend) -> dict[str, Any]:
    request = registration_payload(payload, normalize_email)
    backend.initialize()
    ...
    with backend.transaction() as connection:
        ...
        try:
            duplicate = session.execute(
                f"SELECT 1 FROM customers WHERE LOWER(account_email)=LOWER({ph}) OR LOWER(email)=LOWER({ph})",
                (request["email"], request["email"]),
            ).fetchone()
            identity_duplicate = session.execute(
            ...
            controller_id = _default_controller(session)
            username = _username_for_email(session, dialect, request["email"])
            sftp_username = CustomerIdentityService(backend).allocate_sftp_username(
                session, request["email"]
            )
            identity = insert_customer(
                session,
                backend_name=backend.name,
                parameters=dialect.parameters(8),
            ...
            session.execute(
                f"UPDATE customers SET legal_name={ph},document_type={ph},document_number={ph},"
                f"account_email={ph},sftp_username={ph},registration_status={ph} WHERE id={ph}",
                (
                    request["name"],
                    request["document_type"] or None,
                    request["document_number"] or None,
                    request["email"],
                    sftp_username,
                    "pending",
                    identity.id,
            ...
            active_value = False if backend.name == "postgresql" else 0
            session.execute(
                "INSERT INTO dashboard_users(username,password_hash,role,customer_id,active) "
                f"VALUES ({dialect.parameters(5)})",
                (
                    username,
            ...
            session.execute(
                ...
                f"VALUES ({dialect.parameters(3)})",
                (identity.id, username, "owner"),
            )
            CustomerUserIdentityRepository(backend).set_identity(
                username,
                request["email"],
                verified=False,
    ...
    assert username is not None and identity is not None
    token = CustomerVerificationRepository(backend).create(username)
    delivered = send_verification(request["email"], token)
    audit_customer_event(
        backend,
        username=username,
        action="customer.registration_requested",
        details={
    ...
    if os.environ.get("DSM_CUSTOMER_VERIFICATION_EXPOSE_TOKEN", "").lower() in {
        "1",
        "true",
        "yes",
    }:
...
def request_recovery(payload: dict[str, Any], backend) -> dict[str, Any]:
    ...
    with backend.connect() as connection:
        ...
        try:
            row = session.execute(
                "SELECT u.username FROM dashboard_users u "
                "LEFT JOIN customer_user_identities i ON i.username=u.username "
                "JOIN customers c ON c.id=u.customer_id "
                f"WHERE u.role='customer' AND u.active=TRUE AND "
                f"((i.email IS NOT NULL AND LOWER(i.email)=LOWER({ph})) "
                f"OR (i.email IS NULL AND LOWER(c.account_email)=LOWER({ph}))) "
    ...
    if row is not None:
        ...
        token = CustomerAccountRepository(backend).create_recovery(username)
        audit_customer_event(
            backend,
            username=username,
            action="customer.password_recovery_requested",
        )
        if os.environ.get("DSM_CUSTOMER_RECOVERY_EXPOSE_TOKEN", "").lower() in {
            "1",
            "true",
            "yes",
        }:
...
def reset_password(payload: dict[str, Any], backend) -> dict[str, Any]:
    ...
    username = CustomerAccountRepository(backend).reset_password(
        token, hash_password(password)
    )
    audit_customer_event(
        backend, username=username, action="customer.password_reset"
    )
    return {"reset": True}
...
def _team_snapshot(repository, customer_id, actor_role):
    return {
...
def dispatch_customer_account(method, path, *, payload, user, backend):
    if path not in PUBLIC_PATHS | AUTHENTICATED_PATHS:
        return None
    body = payload or {}
    try:
        if path == "/api/customer/register" and method == "POST":
            return 201, register_customer(body, backend)
        if path == "/api/customer/password-recovery" and method == "POST":
            return 202, request_recovery(body, backend)
        if path == "/api/customer/password-reset" and method == "POST":
            return 200, reset_password(body, backend)
        if path == "/api/customer/members":
            username, customer_id = require_customer(user)
            repository = CustomerTeamRepository(backend)
            actor_role = _actor_role(repository, customer_id, username)
            if method == "GET":
            ...
            if method == "POST":
                ...
                if not re.fullmatch(r"[a-z0-9][a-z0-9._-]{2,63}", target):
                ...
                if action == "create":
                    ...
                    audit_customer_event(
                        backend,
                        username=username,
                        action="customer.member_created",
                        details={"target": target},
                    )
                elif action in {"role", "link"}:
                    role = str(body.get("account_role", "member")).strip().lower()
                    if action == "link":
                        CustomerAccountRepository(backend).set_member(
                            customer_id, target, role
                        )
                    else:
                    ...
                    audit_customer_event(
                        backend,
                        username=username,
                        action="customer.member_role_changed",
                        details={"target": target, "account_role": role},
                    )
                elif action == "access":
                    instance_id = str(body.get("instance_id", "")).strip()
                    profile = (
                        str(body.get("permission_profile", "")).strip().lower()
                        or None
                    )
                    repository.set_instance_access(
                        customer_id, target, instance_id, profile
                    )
                    audit_customer_event(
                        backend,
                        username=username,
                        action="customer.instance_access_changed",
                        instance_id=instance_id,
                        details={"target": target, "permission_profile": profile},
                    )
                elif action == "remove":
                    ...
                    audit_customer_event(
                        backend,
                        username=username,
                        action="customer.member_removed",
                        details={"target": target},
                    )
                else:
        ...
        return 500, {"error": "customer account operation failed"}
