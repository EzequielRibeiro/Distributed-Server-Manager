 ...
 def test_two_instances_share_provider_seed_but_not_working_state(self):
  for platform in ("linux","windows"):
   ...
   with tempfile.TemporaryDirectory() as td:
    ...
    self.assertEqual(a["seed_directories"][0]["source"],str(install));self.assertEqual(b["seed_directories"][0]["source"],str(install));self.assertNotEqual(a["working_directory"],b["working_directory"]);self.assertNotEqual(a["seed_directories"][0]["target"],b["seed_directories"][0]["target"]);self.assertTrue(a["working_directory"].startswith(str(state_a)));self.assertTrue(b["working_directory"].startswith(str(state_b)));self.assertEqual(a["files_root"],a["working_directory"]);self.assertEqual(b["files_root"],b["working_directory"]);self.assertEqual(a["files_root"],a["working_directory"]);self.assertEqual(b["files_root"],b["working_directory"])
