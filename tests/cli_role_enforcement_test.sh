...
[[ "${out}" == "PORTS:show agent-remote" ]] || fail "administrative ports show did not route to Controller surface"
...
[[ "${out}" == "PORTS:set agent-remote 24000 24999 --protocol udp" ]] || fail "hybrid administrative route failed"

if env -u DSM_NODE_ROLE -u CAPIVARA_NODE_ROLE CAPIVARA_AGENT_CONFIG="${TMP}/missing.json" "${TMP}/bin/cap" update check >/dev/null 2>"${TMP}/err"; then fail "unknown role was allowed to dispatch"; fi
