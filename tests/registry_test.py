...
SERVER_SPEC = importlib.util.spec_from_file_location(
    "dashboard_server_registry_test", ROOT / "dashboard" / "server.py"
)
...
class RegistryTest(unittest.TestCase):
    def test_create_aurora_is_complete_and_idempotent(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            first = REGISTRY.create_aurora(root, database)
            second = REGISTRY.create_aurora(root, database)
            self.assertEqual(first["customer_id"], second["customer_id"])
            self.assertEqual(first["customer_code"], "CLI-000001")
            with closing(sqlite3.connect(database)) as connection:
                ...
                self.assertEqual(
                    row,
                    ("controller-demo", "agent-demo", int(first["customer_id"])),
                )
                self.assertEqual(
                ...
                contracts = connection.execute(
                    "SELECT id,game_id,instance_limit FROM service_contracts "
    ...
    def test_customer_creation_consumes_the_contracted_slot(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            database = root / "data" / "capivara.db"
            created = REGISTRY.create_aurora(root, database)
            ...
            with (
                mock.patch.object(
                    ...
                    return_value={"status": "queued", "progress": 5},
                ),
                mock.patch.object(
                    SERVER.subprocess,
                    "run",
            ...
            self.assertEqual(result["instance_id"], "cli-000001-dayz-001")
            ...
            with self.assertRaisesRegex(PermissionError, "contracted instance slot"):
                SERVER.create_customer_instance(
                    user, payload, root=root, database_path=database
                )
    ...
    def test_deleting_instance_removes_runtime_resource_and_releases_contract_slot(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            resource = (
                root / "runtime" / "resources" / "DemoNode" / "minecraft" / "cliente-demo"
            )
            user = aurora_user(created)
            ...
            try:
                with mock.patch.object(
                    SERVER,
                    "control_instance",
                    return_value=(True, {"status": "stopped"}),
                ):
                    result = SERVER.delete_instance(
                        user, instance, database_path=database
                    )
            ...
            (resource / "instance.json").write_text(
                json_customer_metadata(created),
                encoding="utf-8",
            )
            (resource / "server.json").write_text(
                '{"status":"online"}', encoding="utf-8"
            )
            self.assertEqual(SERVER.api_runtime_list(database_path=database), [])
            minecraft_contract = next(
                contract for contract in contracts if contract["game_id"] == "minecraft"
            )
    ...
    def test_purge_orphan_removes_only_a_record_without_instance_directory(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with closing(sqlite3.connect(database)) as connection:
                self.assertIsNone(
                    connection.execute(
                        "SELECT 1 FROM instances WHERE id='dayz-server'"
                    ).fetchone()
                )
    ...
    def test_provisioning_reuses_local_game_data_and_copies_instance_files(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                (instance / "serverfiles" / "server.properties").read_text(),
                "motd=Capivara\n",
            )
            self.assertTrue((instance / "serverfiles" / "server.jar").is_file())
            self.assertIn(
                'RUNTIME_ID="minecraft.java.vanilla"',
                (instance / "instance.conf").read_text(),
            )


def json_customer_metadata(created):
    return (
        '{"controller_id":"controller-demo","agent_id":"agent-demo",'
        f'"customer":{{"id":"{created["customer_code"]}",'
        f'"customer_code":"{created["customer_code"]}"}}}}'
    )
