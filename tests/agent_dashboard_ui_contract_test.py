...
class AgentDashboardUiContractTest(unittest.TestCase):
 ...
 def test_agent_v3_routes_are_registered_in_composition_layer(self):
  ...
  self.assertIn('dashboard/server_part21.py',self.service);self.assertIn('import server_part20 as integration',self.part21);self.assertIn('import server_part19 as integration',self.part20);self.assertIn('import server_part18 as integration',self.part19);self.assertIn('import server_part17 as integration',self.part18);self.assertIn('import server_part14 as integration',self.resource_composition);self.assertIn('import server_part15 as integration',self.file_composition);self.assertIn('import server_part16 as integration',self.latest_composition)
