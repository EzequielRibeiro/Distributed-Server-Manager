...
class CustomerMaintenancePolicySurfaceTest(unittest.TestCase):
 ...
 def test_ui_is_safe_and_capability_driven(self):
  ...
  self.assertIn('renderLoadError',text);self.assertIn('Tentar novamente',text);self.assertIn('let section=document.getElementById("view-maintenance")',text);self.assertIn('maintenanceBound',text)
 ...
 def test_composition_and_static_policy_are_wired(self):
  ...
  page=(ROOT/'dashboard/web/customer-instance.html').read_text(encoding='utf-8');self.assertIn('/customer-maintenance.js?v=4',page);self.assertIn('data-capivara-maintenance="1"',page);self.assertIn('id="maintenance-tab"',page);self.assertIn('id="view-maintenance"',page);self.assertLess(page.index('/customer-maintenance.js?v=4'),page.index('/customer-content-update-policy.js'))
