...
(
    ...
    source "${CORE_INSTALLER}"; chown(){ :; }; chmod(){ :; }
...
if (
    id(){ case "$1" in -un) printf 'node1\n';; -gn) printf 'node1\n';; *) return 1;; esac; }
    source "${CORE_INSTALLER}"; DSM_ROOT="${TMP_DIR}/existing/opt/dsm"; DSM_SOURCE="${ROOT}"; ALLOW_REINSTALL=0
...
(
    id(){ case "$1" in -un) printf 'node1\n';; -gn) printf 'node1\n';; *) return 1;; esac; }
    source "${CORE_INSTALLER}"; DSM_ROOT="${TMP_DIR}/existing/opt/dsm"; DSM_SOURCE="${ROOT}"; ALLOW_REINSTALL=1; guard_existing_installation >/dev/null 2>&1
)
(
