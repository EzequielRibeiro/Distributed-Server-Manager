...
class CustomerRuntimeConsoleLiveTest(unittest.TestCase):
    ...
    def test_customer_runtime_live_script_prefers_sse_with_safe_colored_fallback(self):
        ...
        self.assertIn('id="console-history-panel"', html)
        self.assertIn('id="console-history-summary"', html)
        self.assertIn('Exibir histórico de comandos (0)', html)
        self.assertIn('id="console-history-panel"', controller_html)
        self.assertIn('historyItems=items.filter', script)
        self.assertIn('Exibir histórico de comandos (${historyItems.length})', script)
        self.assertIn('console-history', css)
