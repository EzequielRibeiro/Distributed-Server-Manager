...
class PalworldRuntimeIsolationTest(unittest.TestCase):
 ...
 def test_saved_state_is_private(self):
  ...
  self.assertEqual([{"source":str(shared),"target":str(private),"optional":True}],spec["seed_directories"]);self.assertEqual([{"source":str(private),"target":str(shared)}],spec["bind_paths"]);self.assertEqual("-port=8211",spec["arguments"][0]);self.assertEqual(5,spec["profile_version"]);self.assertEqual({"supported":True,"transport":"palworld-rest"},spec["console"])
 ...
 def test_admin_ports_are_preserved_with_expected_protocols(self):
  ...
  self.assertEqual(["RCONPort","RESTAPIEnabled","RESTAPIPort"],[p["key"] for p in props])
  self.assertEqual(["{{PORT_RCON}}","True","{{PORT_REST_API}}"],[p["value"] for p in props])
 ...
 def test_existing_private_settings_are_not_reseeded(self):
  with tempfile.TemporaryDirectory() as tmp:
   ...
   spec={"working_directory":str(root),"configuration_root":str(config),"catalog_variables":{"PORT_RCON":"3","PORT_REST_API":"4"},"catalog_network_properties":[{"path":"PalWorldSettings.ini","key":"RCONPort","value":"{{PORT_RCON}}","syntax":"ue_option_settings","seed_from":"DefaultPalWorldSettings.ini"},{"path":"PalWorldSettings.ini","key":"RESTAPIEnabled","value":"True","syntax":"ue_option_settings","seed_from":"DefaultPalWorldSettings.ini"},{"path":"PalWorldSettings.ini","key":"RESTAPIPort","value":"{{PORT_REST_API}}","syntax":"ue_option_settings","seed_from":"DefaultPalWorldSettings.ini"}]}
   materialize_network_properties(spec);text=target.read_text(encoding="utf-8")
   self.assertIn("ServerName=Custom",text);self.assertNotIn("ServerName=Default",text);self.assertIn("RCONPort=3",text);self.assertIn("RESTAPIEnabled=True",text);self.assertIn("RESTAPIPort=4",text)
