...
class PalworldRuntimeIsolationTest(unittest.TestCase):
 ...
 def test_saved_state_is_private(self):
  ...
  self.assertEqual([{"source":str(shared),"target":str(private)}],spec["seed_directories"]);self.assertEqual([{"source":str(private),"target":str(shared)}],spec["bind_paths"]);self.assertEqual("-port=8211",spec["arguments"][0])
