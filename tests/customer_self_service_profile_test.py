...
CUSTOMER = {
    ...
    "billing_provider": "secret-provider",
    "billing_customer_id": "secret-billing-id",
}
...
class CustomerSelfServiceProfileTest(unittest.TestCase):
    def setUp(self):
        self.backend = object()
        self.user = {"role": "customer", "scope_id": "CLI-000042", "username": "owner"}
    ...
    def test_get_returns_only_customer_safe_profile(self):
        ...
        self.assertEqual(profile["customer_code"], "CLI-000042")
        self.assertEqual(profile["account_email"], "owner@example.test")
        self.assertNotIn("controller_id", profile)
        self.assertNotIn("billing_provider", profile)
        self.assertNotIn("billing_customer_id", profile)
    ...
    def test_protected_identity_and_admin_fields_are_rejected_before_write(self):
        for field in ("account_email", "customer_code", "controller_id", "registration_status", "billing_status", "username"):
            status, payload = profile_http.customer_profile_update(
                {"changes": {field: "changed"}}, user=self.user, backend=self.backend
            )
            self.assertEqual(status, 400, field)
            self.assertEqual(payload["error"], "protected_fields", field)

    def test_owner_update_reuses_admin_validation_but_only_safe_fields(self):
        result = {
            "updated": True,
            "changed_fields": ["name", "phone"],
            "before": {"name": "Cliente", "phone": "+55"},
            "after": {"name": "Novo Nome", "phone": "+5519"},
            "customer": dict(CUSTOMER, name="Novo Nome", phone="+5519"),
        }
        repo = Mock()
        repo.update.return_value = result
        event_repo = Mock()
        with patch.object(profile_http, "_identity", return_value=(42, dict(CUSTOMER), "owner", "owner")), \
             patch.object(profile_http, "CustomerProfileAdminRepository", return_value=repo), \
             patch.object(profile_http, "audit_customer_event") as audit, \
             patch.object(profile_http, "UniversalEventRepository", return_value=event_repo):
            status, payload = profile_http.customer_profile_update(
                {"changes": {"name": "Novo Nome", "phone": "+5519"}, "correlation_id": "corr-1"},
                user=self.user,
                backend=self.backend,
            )
        self.assertEqual(status, 200)
        repo.update.assert_called_once_with(42, {"name": "Novo Nome", "phone": "+5519"})
        self.assertEqual(payload["correlation_id"], "corr-1")
        self.assertNotIn("controller_id", payload["profile"])
        audit.assert_called_once()
        event = event_repo.publish.call_args.args[0]
        self.assertEqual(event["event_type"], "CUSTOMER_PROFILE_UPDATED")
        self.assertEqual(event["correlation_id"], "corr-1")
        self.assertEqual(event["data"]["changed_fields"], ["name", "phone"])
    ...
    def test_ui_keeps_email_read_only_and_uses_customer_cookie_session(self):
        ...
        self.assertIn("E-mail da conta", script)
