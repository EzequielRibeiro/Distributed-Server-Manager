...
class AgentAdminRemovalTest(unittest.TestCase):
    def setUp(self):
        ...
        self.pairing.enroll(pairing_token=token.token, agent_id=self.agent_id, node_id=self.node_id, name="Agent Removível", fingerprint=self.fingerprint, hostname="remove-host", os_name="linux", architecture="x86_64", address="192.0.2.55")
    ...
    def count(self, table: str, column: str, value: str) -> int:
        with self.backend.connect() as connection:
            row = connection.execute(f"SELECT COUNT(*) AS n FROM {table} WHERE {column}=?", (value,)).fetchone()
        return int(row["n"])
    ...
    def test_remove_is_blocked_while_node_has_instances(self):
        with self.backend.transaction() as connection:
            connection.execute("INSERT INTO instances(id,node_id,game_id,name,status) VALUES (?,?,?,?,?)", ("instance-remove-blocker", self.node_id, "minecraft", "Servidor teste", "offline"))
    ...
    def test_same_identity_can_enroll_again_after_removal(self):
        ...
        reenrolled = self.pairing.enroll(pairing_token=token.token, agent_id=self.agent_id, node_id=self.node_id, name="Agent Reinstalado", fingerprint=self.fingerprint, hostname="remove-host", os_name="linux", architecture="x86_64", address="192.0.2.55")
