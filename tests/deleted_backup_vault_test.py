...
class DeletedBackupVaultTest(unittest.TestCase):
 def setUp(self):
  ...
  with self.backend.transaction() as c:
   c.execute("INSERT INTO nodes(id,name,role,status) VALUES (?,?,?,?)",("controller-node","Controller","controller","active"));c.execute("INSERT INTO nodes(id,name,role,status) VALUES (?,?,?,?)",("agent-node","Agent","agent","active"));c.execute("INSERT INTO controllers(id,node_id,name,status) VALUES (?,?,?,?)",("controller-one","controller-node","Controller","active"));c.execute("INSERT INTO agents(id,controller_id,node_id,name,status) VALUES (?,?,?,?,?)",("agent-one","controller-one","agent-node","Agent","active"));c.execute("INSERT INTO customers(id,controller_id,name,status) VALUES (?,?,?,?)",(1,"controller-one","Customer","active"));c.execute("INSERT INTO instances(id,node_id,game_id,runtime_id,name,status,controller_id,agent_id,customer_id) VALUES (?,?,?,?,?,?,?,?,?)",("instance-one","agent-node","minecraft","minecraft.java.vanilla","Servidor Um","offline","controller-one","agent-one",1))
