...
class PostP0ArchitectureCompletionTest(unittest.TestCase):
    def test_managed_configuration_uses_canonical_precedence(self):
        resolved = resolve_managed_configuration(system={"network": {"port": 2302}},contract={"network": {"port": 2402, "query": 27016}, "slots": 40},customer={"network": {"port": 2502}, "slots": 60, "name": "Customer"})
    ...
    def test_generic_multigame_contract_e2e_native_and_java(self):
        ...
        for path,os_name,arch,engine in cases:
            runtime=load_runtime(path);contract=canonical_from_runtime_v2(runtime);strategy=strategy_from_runtime_contract(contract);games.add(contract["runtime"]["game"])
            self.assertEqual(contract["engine"]["kind"],engine);self.assertTrue(supports_agent(contract,os_name=os_name,architecture=arch));self.assertEqual(strategy["runtime_id"],contract["runtime"]["id"]);self.assertFalse(strategy["layout"]["working_dir"].startswith(("/","\\")))
    ...
    def test_forge_is_generic_java_runtime_on_supported_provider(self):
        ...
        self.assertEqual(runtime["id"],"minecraft.java.forge");self.assertEqual(runtime["process"]["engine"],"java");self.assertEqual(runtime["artifact"]["provider"],"http");self.assertEqual(runtime["version"]["resolver"],"forge_maven")
        ...
        self.assertIn(runtime["id"],{row["id"] for row in matrix["published_runtimes"]})
        self.assertNotIn(runtime["id"],{row["id"] for row in matrix["deferred_runtimes"]})
