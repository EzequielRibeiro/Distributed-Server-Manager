#!/usr/bin/env python3
"""Windows Agent backup must operate on instance-private state."""
...
def _load_backup_client():
    module_path = ROOT / "agents/windows/runtime/backup_client.py"
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "capivara_windows_backup_client_root_test",
            module_path,
        )
...
class WindowsBackupClientInstanceRootTest(unittest.TestCase):
    def setUp(self):
        ...
        self.record = {
            ...
            # Provider/game content shared between instances.
    ...
    def command(self):
        return {
            "command_id": "windows-backup-command-safe",
            "instance_id": "instance-safe",
            "action": "create",
            "policy": {
                "mode": "full",
    ...
    def test_backup_root_prefers_files_root(self):
        with patch.object(
            backup_client,
            "get_instance",
    ...
    def test_create_does_not_archive_shared_game_content(self):
        with patch.object(
            backup_client,
            "get_instance",
            return_value=self.record,
    ...
    def test_restore_replaces_only_private_state(self):
        with patch.object(
            backup_client,
            "get_instance",
            return_value=self.record,
