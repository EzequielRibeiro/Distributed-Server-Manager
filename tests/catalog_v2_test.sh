...
[[ "$(jq 'length' <<<"${RUNTIMES}")" -eq 12 ]] || fail "expected twelve installable Minecraft environments"
jq -e 'map(.id) | index("minecraft.bedrock.vanilla") != null' <<<"${RUNTIMES}" >/dev/null || fail "Bedrock runtime missing"
jq -e 'map(.id) | index("minecraft.java.folia") != null' <<<"${RUNTIMES}" >/dev/null || fail "Folia runtime missing"
...
jq -e 'map(.id) | index("minecraft.java.purpur") != null' <<<"${RUNTIMES}" >/dev/null || fail "Purpur runtime missing"
jq -e 'map(.id) | index("minecraft.java.quilt") != null' <<<"${RUNTIMES}" >/dev/null || fail "Quilt runtime missing"
jq -e 'map(.id) | index("minecraft.java.spongevanilla") != null' <<<"${RUNTIMES}" >/dev/null || fail "SpongeVanilla runtime missing"
jq -e 'map(.id) | index("minecraft.java.youer") != null' <<<"${RUNTIMES}" >/dev/null || fail "Youer runtime missing"
...
[[ "$(jq 'length' <<<"${API_RUNTIMES}")" -eq 12 ]] || fail "dashboard API runtime adapter invalid"
