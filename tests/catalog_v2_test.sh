...
[[ "$(jq 'length' <<<"${RUNTIMES}")" -eq 11 ]] || fail "expected eleven installable Minecraft environments"
...
[[ "$(jq 'length' <<<"${API_RUNTIMES}")" -eq 11 ]] || fail "dashboard API runtime adapter invalid"
