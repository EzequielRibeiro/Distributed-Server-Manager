...
class D2RepositoryTest(unittest.TestCase):
 def setUp(self):
  ...
  with self.backend.transaction() as c:
   c.execute("INSERT INTO nodes(id,name,role) VALUES (?,?,?)",("controller-node","Controller","controller"));c.execute("INSERT INTO nodes(id,name,role) VALUES (?,?,?)",("agent-node","Agent","agent"));c.execute("INSERT INTO controllers(id,node_id,name) VALUES (?,?,?)",("controller-d2","controller-node","D2"));c.execute("INSERT INTO agents(id,controller_id,node_id,name,status) VALUES (?,?,?,?,?)",("agent-d2","controller-d2","agent-node","Agent D2","active"));customer=c.execute("INSERT INTO customers(controller_id,name,status) VALUES (?,?,?)",("controller-d2","Customer","active"));customer_id=customer.lastrowid;c.execute("INSERT INTO instances(id,node_id,game_id,name,status,controller_id,agent_id,customer_id) VALUES (?,?,?,?,?,?,?,?)",("instance-d2","agent-node","game.generic","Generic","offline","controller-d2","agent-d2",customer_id))
