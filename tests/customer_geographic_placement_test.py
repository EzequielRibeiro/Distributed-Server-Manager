...
class CustomerGeographicPlacementTest(unittest.TestCase):
    def test_customer_receives_only_public_logical_locations(self):
        with patch("customer_placement_locations.LocationRepository", _Repository), \
            ...
            payload = customer_placement_locations(
                ...
                object(), game_id="dayz", runtime_id="dayz.stable", contract_id="contract-1",
                client_latitude=-23.55, client_longitude=-46.63,
            )
    ...
    def test_http_rejects_invalid_coordinates_without_exposing_details(self):
        status, payload = dispatch_customer_placement_locations_get(
            ...
            user={"role": "customer", "scope_id": "CLI-000042"}, backend=object(), query={"latitude": ["not-a-number"]},
        )
