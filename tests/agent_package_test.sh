...
# The installer intentionally discovers top-level runtime Python modules
# dynamically. Validate that contract instead of requiring every module name
# to be duplicated literally in install-agent.sh.
grep -Fq 'mapfile -t RUNTIME_FILES' "${INSTALLER:-${PACKAGE}/install-agent.sh}" || fail "installer does not dynamically discover runtime files"
grep -Fq 'find "${PACKAGE_DIR}/agent/runtime"' "${INSTALLER:-${PACKAGE}/install-agent.sh}" || fail "installer does not scan packaged runtime directory"
grep -Fq "name '*.py'" "${INSTALLER:-${PACKAGE}/install-agent.sh}" || fail "installer runtime discovery is not limited to Python modules"
grep -Fq 'for file in "${RUNTIME_FILES[@]}"' "${INSTALLER:-${PACKAGE}/install-agent.sh}" || fail "installer does not install discovered runtime files"
