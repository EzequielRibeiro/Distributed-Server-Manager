...
class WindowsAgentStorageParityTest(unittest.TestCase):
 ...
 def _migration_command(self):return {"migration_id":"migration-one","agent_id":"agent-win","instance_id":"instance-one","source_storage_pool_id":"source","target_storage_pool_id":"target","action":"migrate"}
 def _cleanup_command(self):return {"migration_id":"cleanup-one","source_migration_id":"migration-one","agent_id":"agent-win","instance_id":"instance-one","source_storage_pool_id":"source","target_storage_pool_id":"target","action":"cleanup-source","expected_verified_files":1,"expected_verified_bytes":3}
 def test_migration_copies_verifies_switches_and_preserves_source(self):
  src=self.source/"instance-one";src.mkdir();(src/"a.cfg").write_bytes(b"abc");record=self._record("source");registered=[]
  with patch.object(self.executor.instance_runtime,"_owned",return_value=record),patch.object(self.executor.instance_runtime,"status",return_value={"observed_state":"stopped"}),patch.object(self.executor,"runtime_operation",return_value=nullcontext()),patch.object(self.executor,"emit_runtime_event",return_value={}),patch.object(self.executor.runtime_materialization,"materialize",return_value={}),patch.object(self.executor.instance_runtime,"register_instance",side_effect=lambda value:registered.append(dict(value)) or value):result=self.executor.execute(self.config,self._migration_command(),self.root/"result.json")
  self.assertEqual(result["status"],"completed");self.assertTrue((src/"a.cfg").exists());self.assertEqual((self.target/"instance-one"/"a.cfg").read_bytes(),b"abc");self.assertEqual(registered[-1]["storage_pool_id"],"target");self.assertEqual(result["verified_bytes"],3)
 def test_migration_rejects_symlink_before_copy(self):
  src=self.source/"instance-one";src.mkdir();outside=self.root/"outside.txt";outside.write_text("secret",encoding="utf-8")
  try:(src/"unsafe").symlink_to(outside)
  except (OSError,NotImplementedError):self.skipTest("symlinks unavailable")
  record=self._record("source")
  with patch.object(self.executor.instance_runtime,"_owned",return_value=record),patch.object(self.executor.instance_runtime,"status",return_value={"observed_state":"stopped"}),patch.object(self.executor,"runtime_operation",return_value=nullcontext()),patch.object(self.executor,"emit_runtime_event",return_value={}):result=self.executor.execute(self.config,self._migration_command(),self.root/"symlink-migration.json")
  self.assertEqual(result["status"],"failed");self.assertIn("symlink",result["error"].lower());self.assertFalse((self.target/"instance-one").exists())
 def test_cleanup_only_removes_preserved_source_after_target_switch(self):
  src=self.source/"instance-one";dst=self.target/"instance-one";src.mkdir();dst.mkdir();(src/"a.cfg").write_bytes(b"abc");(dst/"a.cfg").write_bytes(b"abc");record=self._record("target")
  with patch.object(self.executor.instance_runtime,"_owned",return_value=record),patch.object(self.executor,"runtime_operation",return_value=nullcontext()),patch.object(self.executor,"emit_runtime_event",return_value={}):result=self.executor.execute(self.config,self._cleanup_command(),self.root/"cleanup.json")
  self.assertEqual(result["status"],"completed");self.assertFalse(src.exists());self.assertTrue(dst.exists());self.assertEqual(result["removed_bytes"],3)
 def test_cleanup_rejects_symlink_and_wrong_runtime_pool(self):
  src=self.source/"instance-one";dst=self.target/"instance-one";src.mkdir();dst.mkdir();record=self._record("source");command=self._cleanup_command()
  ...
  self.assertEqual(result["status"],"failed");self.assertTrue(src.exists());record=self._record("target")
  try:(src/"unsafe").symlink_to(self.root/"outside")
  except (OSError,NotImplementedError):self.skipTest("symlinks unavailable")
  command.pop("expected_verified_files",None);command.pop("expected_verified_bytes",None)
