...
def supported_profiles(runtime_root:Path)->set[str]:
 env=os.environ.copy();env["PYTHONPATH"]=str(runtime_root)
 cp=subprocess.run([sys.executable,"-c","from profiles.registry import supported_profiles; print('\\n'.join(supported_profiles()))"],cwd=ROOT,env=env,capture_output=True,text=True,check=False)
 if cp.returncode:
...
class CatalogAgentRuntimeProfileParityTest(unittest.TestCase):
 ...
 def test_windows_does_not_claim_primitive_only_runtimes(self):
  self.assertIn("dayz.stable",self.windows)
  for runtime_id in ("rust.stable","arma3.stable","minecraft.java.vanilla","mindustry.github"):
   self.assertNotIn(runtime_id,self.windows)
 def test_deferred_runtime_is_not_published(self):
  published={str(item.get("id") or "").lower() for item in self.matrix.get("published_runtimes",[])}
  deferred={str(item.get("id") or "").lower() for item in self.matrix.get("deferred_runtimes",[])}
  self.assertFalse(published & deferred)
