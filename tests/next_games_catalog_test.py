...
class NextGamesCatalogTest(unittest.TestCase):
 def test_supported_and_deferred_decisions_are_explicit(self):
  ...
  self.assertTrue({"sevendaystodie.stable","satisfactory.stable","garrysmod.stable","factorio.stable","left4dead2.stable","armareforger.stable","theisle.stable","arksurvivalascended.stable","valheim.stable","fivem.stable","luanti.stable"}<=published);self.assertEqual(deferred,set());self.assertTrue(published.isdisjoint(deferred))
 def test_artifact_contracts_match_known_dedicated_distributions(self):
  ...
  fivem=load(ROOT/"catalog/v2/games/fivem/runtimes/stable.json");self.assertEqual(fivem["artifact"]["provider"],"fivem");self.assertEqual(fivem["process"]["executable"],"server/run.sh");self.assertEqual(fivem["requirements"]["os"],["linux"])
  luanti=load(ROOT/"catalog/v2/games/luanti/runtimes/stable.json");self.assertEqual(luanti["artifact"]["provider"],"http-archive");self.assertEqual(luanti["version"]["value"],"5.17.0");self.assertEqual(luanti["installation"]["installer"]["type"],"cmake_source");self.assertEqual(luanti["requirements"]["os"],["linux"])
 def test_protocols_are_deterministic(self):
  for game in ("sevendaystodie","satisfactory","garrysmod","left4dead2","theisle","luanti"):
 ...
 def test_registry_exposes_intended_profiles(self):
  ...
  for key in ("sevendaystodie.stable","factorio.stable","armareforger.stable","satisfactory.stable","garrysmod.stable","left4dead2.stable","theisle.stable","valheim.stable","fivem.stable","luanti.stable"):self.assertIn(key,names)
  self.assertNotIn("arksurvivalascended.stable",names)
