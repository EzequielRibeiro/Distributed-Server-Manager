...
class _FakeContent:
 def __init__(self,revisions,versions=None):self.revisions=dict(revisions);self.versions=dict(versions or {})
 def get(self,instance_id,content_id):return {'instance_id':instance_id,'content_id':content_id,'revision':self.revisions[content_id],'version':self.versions.get(content_id)}


class _FakeService:
 def __init__(self,revisions,fail=None,versions=None):self.content=_FakeContent(revisions,versions);self.calls=[];self.fail=set(fail or [])
