...
class ContentUpdateDispatchRepository:
 ...
 def due(self,*,limit:int=200,now:datetime|None=None,retry_seconds:int=300)->list[dict[str,Any]]:
  count=max(1,min(int(limit),1000));current=(now or datetime.now(timezone.utc)).astimezone(timezone.utc);ph=self.dialect.placeholder
  sql=("SELECT u.agent_id,u.instance_id,u.content_id,u.installed_version,u.available_version,u.dispatched_available_version,u.dispatched_assignment_revision,"+
       ...
       "FROM content_update_state u JOIN content_assignments a ON a.instance_id=u.instance_id AND a.content_id=u.content_id AND a.agent_id=u.agent_id " +
 ...
 def claim(self,item:dict[str,Any])->bool:
  aid=str(item.get('agent_id') or '');iid=str(item.get('instance_id') or '');cid=str(item.get('content_id') or '');available=str(item.get('available_version') or '');revision=int(item.get('assignment_revision') or 0);old_attempt=item.get('dispatch_attempted_at');ph=self.dialect.placeholder;now=utc_timestamp()
  if not aid or not iid or not cid or not available or revision<1:return False
  with self.session(transaction=True) as session:
   current=session.execute(f'SELECT revision,desired_state,agent_id FROM content_assignments WHERE instance_id={ph} AND content_id={ph}',(iid,cid)).fetchone()
   if not current or str(current['agent_id'] or '')!=aid or str(current['desired_state'] or '')!='installed' or int(current['revision'] or 0)!=revision:return False
   if old_attempt is None:
    sql=f"UPDATE content_update_state SET dispatch_attempted_at={ph},dispatch_attempted_version={ph},dispatch_attempted_assignment_revision={ph},dispatch_error=NULL,updated_at={ph} WHERE agent_id={ph} AND instance_id={ph} AND content_id={ph} AND state='update_available' AND available_version={ph} AND dispatch_attempted_at IS NULL"
    cursor=session.execute(sql,(now,available,revision,now,aid,iid,cid,available))
   else:
    sql=f"UPDATE content_update_state SET dispatch_attempted_at={ph},dispatch_attempted_version={ph},dispatch_attempted_assignment_revision={ph},dispatch_error=NULL,updated_at={ph} WHERE agent_id={ph} AND instance_id={ph} AND content_id={ph} AND state='update_available' AND available_version={ph} AND dispatch_attempted_at={ph}"
    cursor=session.execute(sql,(now,available,revision,now,aid,iid,cid,available,old_attempt))
