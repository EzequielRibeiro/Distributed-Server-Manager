...
def main(argv=None):
    ...
    options=SSHDeployOptions(host=a.host,ssh_user=a.ssh_user,ssh_port=a.ssh_port,identity_file=a.identity_file,connect_timeout=a.connect_timeout,password_file=a.password_file)
    try:r=(preflight_windows_ssh if a.platform=="windows" else preflight_ssh)(options)
        ...
        p.exit(2,f"Erro: {exc}\n")
    out={"ok":True,"host":a.host,"ssh_port":a.ssh_port,"platform":r.get("platform"),"architecture":r.get("architecture"),"transport":"openssh","authentication":"password-file" if a.password_file else "ssh-key-or-agent","privilege":r.get("privilege")}
    if a.json:print(json.dumps(out,ensure_ascii=False,sort_keys=True))
    else:
        print("Capivara Agent Connection Test\n");print(f"Host.............. {a.host}:{a.ssh_port}");print("SSH............... OK");print(f"Platform.......... {out['platform']}");print(f"Architecture...... {out['architecture']}");print(f"Authentication.... {out['authentication']}");print(f"Privilege......... {out.get('privilege') or 'administrator'}");print("Ready............. YES")
    return 0
