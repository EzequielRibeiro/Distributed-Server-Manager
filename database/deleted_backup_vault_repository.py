...
ACTIVE={"backup_pending","export_pending","removal_pending"};FINAL={"ready","downloaded","expired","failed","removal_failed"}
...
class DeletedBackupVaultRepository:
 ...
 def start(self,instance_id,*,requested_by,retention_hours=168):
  ...
  with self.session() as s:
   row=s.execute(f"SELECT vault_id FROM deleted_instance_backups WHERE source_instance_id={self.ph} AND status IN ('backup_pending','export_pending','removal_pending') ORDER BY created_at DESC LIMIT 1",(str(instance_id),)).fetchone()
  if row is not None:return self.get(str(row["vault_id"])),True
  job=self.backups.request(str(instance_id),action="create",reason="final_delete",requested_by=str(requested_by or "customer"));vault_id="vault-"+uuid.uuid4().hex;now=_now();expires=_iso(now+timedelta(hours=max(1,min(int(retention_hours),168))))
  manifest={"schema_version":1,"kind":"CapivaraDeletedInstanceBackup","source_instance_id":str(instance["id"]),"source_instance_name":instance.get("name"),"game_id":instance.get("game_id"),"runtime_id":instance.get("runtime_id")}
 ...
 def reconcile(self,vault_id):
  ...
  if status in {"downloaded","expired","failed","ready"}:return item
  if status=="backup_pending":
   ...
   filename=Path(str(job.get("artifact_path") or f"{job['backup_id']}.tar.gz")).name
   transfer=self.artifacts.create(agent_id=str(item["agent_id"]),instance_id=str(item["source_instance_id"]),customer_id=int(item["customer_id"]),direction="agent_to_controller",purpose="deleted_backup_export",filename=filename,source_ref=str(job["backup_id"]),requested_by=str(item.get("requested_by") or ""),ttl_hours=168)
   item=self._set(vault_id,status="export_pending",backup_id=str(job["backup_id"]),transfer_id=str(transfer["transfer_id"]));status="export_pending"
  if status=="export_pending":
   ...
   artifact,_=self.artifacts.controller_artifact(str(transfer["transfer_id"]));remove=self.runtime.enqueue(agent_id=str(item["agent_id"]),instance_id=str(item["source_instance_id"]),action="remove",requested_by=str(item.get("requested_by") or ""))
   item=self._set(vault_id,status="removal_pending",artifact_path=str(artifact),size_bytes=transfer.get("size_bytes"),sha256=transfer.get("sha256"),remove_command_id=str(remove["command_id"]));status="removal_pending"
 ...
 def list_for_customer(self,customer_id):
  ...
  with self.session() as s:rows=s.execute(f"SELECT vault_id FROM deleted_instance_backups WHERE customer_id={self.ph} AND status NOT IN ('downloaded','expired') ORDER BY created_at DESC",(cid,)).fetchall()
  output=[]
  for row in rows:
 ...
 def complete_download(self,vault_id,customer_id):
  path,item=self.artifact_for_customer(vault_id,customer_id);path.unlink(missing_ok=True)
  try:path.parent.rmdir()
  except OSError:pass
  return self._set(vault_id,status="downloaded",downloaded_at=_iso(_now()),artifact_path=None)
 def cleanup_expired(self):
  now=_now();count=0
  with self.session() as s:rows=s.execute("SELECT vault_id,artifact_path,expires_at FROM deleted_instance_backups WHERE status NOT IN ('downloaded','expired') AND expires_at IS NOT NULL").fetchall()
  for row in rows:
