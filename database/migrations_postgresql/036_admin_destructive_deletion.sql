-- Capivara DSM - Migration 036 - PostgreSQL
-- Add explicit transitional contract deletion state and Agent runtime remove action.

-- Replace the generated CHECK on service_contracts.status without depending on
-- its backend-generated constraint name.
DO $$
DECLARE
...
-- Rebuild the command table so the immutable action allowlist includes remove.
