...
CREATE TABLE agent_runtime_inventory (
 ...
 hostname TEXT, os_name TEXT, architecture TEXT, capivara_version TEXT,
 address TEXT, fingerprint TEXT, capabilities_json TEXT NOT NULL DEFAULT '{}',
 cpu_json TEXT NOT NULL DEFAULT '{}', ram_total_bytes BIGINT,
 storage_json TEXT NOT NULL DEFAULT '{}', health_status TEXT NOT NULL DEFAULT 'offline'
 CHECK (health_status IN ('online','degraded','offline')), last_seen TIMESTAMPTZ,
...
CREATE INDEX idx_agent_runtime_health ON agent_runtime_inventory(health_status,last_seen);
CREATE INDEX idx_agent_runtime_fingerprint ON agent_runtime_inventory(fingerprint);
CREATE TABLE agent_pairing_tokens (
 id TEXT PRIMARY KEY, controller_id TEXT NOT NULL REFERENCES controllers(id) ON DELETE CASCADE,
...
CREATE INDEX idx_agent_pairing_tokens_controller ON agent_pairing_tokens(controller_id,expires_at,consumed_at);
CREATE TABLE agent_credentials (
 ...
 credential_type TEXT NOT NULL DEFAULT 'opaque-v1', secret_hash TEXT, fingerprint TEXT NOT NULL,
 public_key TEXT, status TEXT NOT NULL DEFAULT 'active' CHECK (status IN ('active','revoked')),
 issued_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, last_used_at TIMESTAMPTZ, revoked_at TIMESTAMPTZ,
 UNIQUE(agent_id,id)
);
CREATE INDEX idx_agent_credentials_agent ON agent_credentials(agent_id,status);
CREATE INDEX idx_agent_credentials_fingerprint ON agent_credentials(fingerprint,status);
...
CREATE TABLE IF NOT EXISTS backup_jobs (
 command_id TEXT PRIMARY KEY, backup_id TEXT, instance_id TEXT NOT NULL REFERENCES instances(id) ON DELETE CASCADE,
