...
class InstanceBackupCloneRepository:
 ...
 def validate_source(self,vault_id,customer_id,game_id,runtime_id):
  source=self.vault.get(str(vault_id))
 ...
 def reconcile(self,clone_id):
  ...
  if status=="provisioning":
   ...
   source,path=self.validate_source(item["source_vault_id"],item["customer_id"],self._target_game(item["target_instance_id"]),self._target_runtime(item["target_instance_id"]))
  ...
  if status=="transferring":
   ...
   if state=="failed":return self._set(clone_id,status="failed",last_error=str(transfer.get("last_error") or "backup transfer failed")[:1024])
   if state!="completed":return item
   job=self.backups.request(str(item["target_instance_id"]),action="restore",backup_id=str(item["imported_backup_id"]),reason="create_from_backup",requested_by=str(item.get("requested_by") or ""));item=self._set(clone_id,status="restoring",restore_job_id=str(job["command_id"]));status="restoring"
  if status=="restoring":
   ...
   if state=="completed":return self._set(clone_id,status="completed",completed_at=_now())
  return item
