...
class AgentInstanceProvisioningRepository:
        ...
        *,
        ...
        with self.session(transaction=True) as session:
            ...
            instance = session.execute(
                f"SELECT id,agent_id,node_id,game_id,runtime_id FROM instances WHERE id={ph}",
                (instance_id,),
            ).fetchone()
            if instance is None:
            ...
            existing = session.execute(
                ...
                f"WHERE instance_id={ph} AND status IN ('queued','delivered','running') ORDER BY created_at ASC LIMIT 1",
                (instance_id,),
            ).fetchone()
            ...
            rows = session.execute(
                ...
                f"WHERE instance_id={ph} ORDER BY name,protocol,port",
                (instance_id,),
            ).fetchall()
            ports = {
                str(row["name"]): {
                    ...
                    "bind_address": str(row["bind_address"] or "0.0.0.0"),
                }
                for row in rows
            }
            if not ports:
            ...
            request = {
                ...
                "instance": {
                    "instance_id": instance_id,
                    "agent_id": agent_id,
                    "game_id": str(instance["game_id"] or ""),
                    "environment_id": environment_id,
                    "runtime_id": str(instance["runtime_id"] or instance_id),
                    "desired_state": desired_state,
                },
                "content": {"action": "install", "selection": selection},
                "ports": ports,
            ...
            session.execute(
                ...
                "provisioning_id,agent_id,instance_id,environment_id,selector,request_json,status,current_step,progress,requested_by,created_at,updated_at"
                f") VALUES ({self.dialect.parameters(12)})",
                (
                    provisioning_id, agent_id, instance_id, environment_id, selector,
                    json.dumps(request, separators=(",", ":"), sort_keys=True),
                    "queued", "queued", 0, str(requested_by or "").strip() or None, now, now,
                ),
            )
    ...
    def snapshot(self, provisioning_id: str) -> dict[str, Any]:
        ...
        with self.session() as session:
            row = session.execute(
                f"SELECT * FROM agent_instance_provisioning WHERE provisioning_id={ph}",
                (provisioning_id,),
            ).fetchone()
        if row is None:
    ...
    def mark_delivered(self, provisioning_id: str) -> dict[str, Any]:
        ...
        with self.session(transaction=True) as session:
            session.execute(
                ...
                f"WHERE provisioning_id={ph} AND status IN ('queued','delivered','running')",
    ...
    def apply_result(self, agent_id: str, result: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        with self.session(transaction=True) as session:
            if status == "running":
                ...
                session.execute(
                    "UPDATE agent_instance_provisioning SET "
                    f"status={ph},current_step={ph},progress={ph},result_json={ph},last_error={ph},completed_at={ph},updated_at={ph} "
                    f"WHERE provisioning_id={ph} AND status NOT IN ('completed','failed')",
                    (status, current_step, progress, payload, error, now, now, provisioning_id),
                )
        return self.snapshot(provisioning_id)
