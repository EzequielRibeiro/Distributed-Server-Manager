...
class AgentInstanceProvisioningRepository:
    ...
    def _publish_event(
        ...
        *,
        ...
        """Persist a canonical provisioning event directly in Universal Events."""
        ...
        self.events.publish(
            {
                ...
                "source_id": "controller.provisioning",
                "severity": severity,
        ...
        *,
        ...
        with self.session(transaction=True) as session:
            ...
            request = {
                ...
                "content": {"action": "ensure", "selection": selection},
                "ports": ports,
        ...
        self._publish_event(
            event_id=f"{provisioning_id}:queued",
            event_type="INSTANCE_PROVISION_QUEUED",
            severity="info",
            provisioning=state,
            data={"message": "Provisionamento da instância foi enfileirado para o Agent."},
        )
        return state
    ...
    def mark_delivered(self, provisioning_id: str) -> dict[str, Any]:
        ...
        with self.session(transaction=True) as session:
            session.execute(
                ...
                f"updated_at={ph} WHERE provisioning_id={ph} "
        ...
        with self.session(transaction=True) as session:
            if status == "running":
                ...
                session.execute(
                    "UPDATE agent_instance_provisioning SET "
                    f"status={ph},current_step={ph},progress={ph},result_json={ph},last_error={ph},"
                    f"completed_at={ph},updated_at={ph} WHERE provisioning_id={ph} "
