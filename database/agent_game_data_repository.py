...
class AgentGameDataRepository:
    ...
    def enqueue(
        ...
        *,
        ...
        with self.session(transaction=True) as session:
            ...
            session.execute(
                "INSERT INTO agent_game_data_jobs("
                "job_id,agent_id,action,environment_id,selector,selection_json,status,progress,requested_by,created_at,updated_at"
    ...
    def snapshot(self, job_id: str) -> dict[str, Any]:
        ...
        with self.session() as session:
            row = session.execute(
                f"SELECT * FROM agent_game_data_jobs WHERE job_id={ph}", (job_id,)
            ).fetchone()
        if row is None:
    ...
    def apply_result(self, agent_id: str, result: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        try:
            progress_value = int(progress if progress is not None else (100 if status == "completed" else current.get("progress", 0)))
        except (TypeError, ValueError):
            progress_value = int(current.get("progress", 0) or 0)
        ...
        with self.session(transaction=True) as session:
            if status == "running":
                session.execute(
                    ...
                    f"result_json={ph},last_error={ph},updated_at={ph} "
                    f"WHERE job_id={ph} AND status NOT IN ('completed','failed')",
                    (status, progress_value, now, result_json, error, now, job_id),
                )
            else:
                session.execute(
                    "UPDATE agent_game_data_jobs SET "
                    ...
                    f"WHERE job_id={ph} AND status NOT IN ('completed','failed')",
                    (status, progress_value, result_json, error, now, now, job_id),
                )
        return self.snapshot(job_id)
