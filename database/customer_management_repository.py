...
class CustomerManagementRepository:
    ...
    def _username(value: Any) -> str:
        ...
        if not _USERNAME_RE.fullmatch(username):
            raise ValueError("username must contain 2-128 letters, numbers, dot, underscore or hyphen")
        return username
        ...
        with self.backend.connect() as connection:
            ...
            try:
                rows = session.execute(
                    "SELECT id,name,status FROM controllers WHERE status='active' ORDER BY name,id"
                ).fetchall()
            finally:
                session.close()
        return {
    ...
    def create_account(
        ...
        *,
        ...
        """Create Customer + owner login + identity + password state atomically."""
        self.initialize()
        ...
        with self.admin.session(transaction=True) as session:
            ...
            if session.execute(
                f"SELECT 1 FROM customers WHERE LOWER(COALESCE(account_email,''))=LOWER({ph})",
                (email,),
            ).fetchone() is not None:
                raise ValueError("e-mail already exists")
            ...
            session.execute(
                ...
                (username, hash_password(temporary_password), "customer", identity.id, active_value),
            )
            session.execute(
    ...
    def search(self, query: str = "", field: str = "all") -> list[dict[str, Any]]:
        ...
        conditions: dict[str, tuple[str, tuple[Any, ...]]] = {
            "name": (
                "(LOWER(c.name) LIKE " + ph + " OR LOWER(COALESCE(c.legal_name,'')) LIKE " + ph + ")",
                (like, like),
            ),
            "email": (
                "(LOWER(COALESCE(c.account_email,c.email,'')) LIKE " + ph +
                " OR EXISTS (SELECT 1 FROM customer_user_identities i WHERE i.username=u.username AND LOWER(i.email) LIKE " + ph + "))",
                (like, like),
            ),
            "document": (
        ...
        if field == "all":
            pieces = [conditions[key][0] for key in ("name", "email", "document", "customer_code", "username")]
            params: tuple[Any, ...] = tuple(
                item for key in ("name", "email", "document", "customer_code", "username")
                for item in conditions[key][1]
            )
        ...
        for row in rows:
            ...
            item = grouped.setdefault(customer_id, {
                "id": customer_id,
        ...
        with self.backend.connect() as connection:
            ...
            try:
                ...
                contracts = session.execute(
                    "SELECT c.id,c.game_id,c.status,c.instance_limit,c.starts_at,c.ends_at,c.created_at,c.updated_at,c.metadata_json,"
                    ...
                    "GROUP BY c.id,c.game_id,c.status,c.instance_limit,c.starts_at,c.ends_at,c.created_at,c.updated_at,c.metadata_json "
