#!/usr/bin/env bash
# =============================================================
# Capivara DSM - PaperMC Version Resolver v1
#
...
paper_list()
{
    ...
    jq -c \
      ...
      --arg variant "${VARIANT_ID:-paper}" \
      --arg project "${PAPERMC_PROJECT}" '
      [
...
{
    local SELECTOR="${1:-}"
    local VERSION BUILD RAW SELECTED
    ...
    if jq -e '.ok == false' >/dev/null 2>&1 <<<"${RAW}"
    then
    ...
    if [[ -n "${BUILD}" ]]
    then
        SELECTED="$(jq -c --arg build "${BUILD}" '
          map(select((.id|tostring)==$build or (.number?|tostring)==$build)) | first // empty
        ' <<<"${RAW}")"
    else
        SELECTED="$(jq -c '
          (map(select(.channel == "STABLE")) | first) // empty
        ' <<<"${RAW}")"
    fi
    ...
    then
        jq -nc --arg version "${VERSION}" --arg build "${BUILD}" '
          {error:"build_not_found",version:$version,build:(if $build=="" then "latest-stable" else $build end)}'
        return 1
    fi
