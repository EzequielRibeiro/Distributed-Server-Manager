...
planner_visit()
{
    ...
    if [[ "${PLANNER_VISITING[${ID}]:-0}" == 1 ]]; then
        planner_error "Dependency cycle detected at ${ID}."
        return 1
    fi
    ...
    while IFS= read -r DEP; do
        [[ -n "${DEP}" ]] || continue
        planner_visit "${DEP}" || return 1
    done < <(jq -r '.dependencies[]? | select(.required != false) | .id' "${FILE}" | tr -d '\r')
...
planner_plan()
{
    ...
    if [[ "$(jq -r '.compatible' <<<"${RESULT}" | tr -d '\r')" != true ]]; then
        jq . <<<"${RESULT}"
        return 1
    fi
    ...
    for ID in "${PLANNER_ORDER[@]}"; do
                ...
                OPERATIONS="$(jq -c --argjson ops "${OPERATIONS}" '
                    . as $d | $ops + [{
                        ...
                        version:$d.version,
                        target:(
                            if $d.content_type == "plugin" then
                                "plugins"
                            elif $d.content_type == "mod" then
                                "mods"
                            elif $d.content_type == "modpack" then
                                "."
                            elif $d.content_type == "datapack" then
                                "world/datapacks"
                            else
