...
_TYPES={"mod","plugin","modpack","map","asset","workshop","other"}
_STATES={"installed","absent"}
_PROVIDERS={"steam","http","http-archive","github","local","custom","source-build"}
...
def normalize_assignment(raw:Mapping[str,Any],*,expected_agent_id:str|None=None)->dict[str,Any]:
    ...
    instance=_token(raw.get("instance_id"),"instance_id"); content=_token(raw.get("content_id"),"content_id")
    game=_token(raw.get("game_id"),"game_id").lower(); ctype=str(raw.get("content_type") or "other").strip().lower()
    ...
    version=str(raw.get("version") or "latest").strip()[:191] or "latest"
    provider=str(raw.get("provider") or (raw.get("artifact") or {}).get("provider") or "").strip().lower()
    ...
    target=_target(raw.get("target") or {"mod":"mods","plugin":"plugins","modpack":"modpacks","map":"maps"}.get(ctype,"assets"))
    deps=[_token(v,"dependency") for v in (raw.get("dependencies") or [])][:200]
    conflicts=[_token(v,"conflict") for v in (raw.get("conflicts") or [])][:200]
    identity={"agent_id":agent,"instance_id":instance,"content_id":content,"game_id":game,"content_type":ctype,"desired_state":state,"version":version,"provider":provider,"target":target,"artifact":artifact,"dependencies":deps,"conflicts":conflicts}
    checksum=hashlib.sha256(_j(identity).encode()).hexdigest()
