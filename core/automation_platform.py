...
def normalize_rule(raw:Mapping[str,Any])->dict[str,Any]:
 ...
 rule_id=_token(raw.get("rule_id"),"rule_id");name=str(raw.get("name") or rule_id).strip()[:191];enabled=bool(raw.get("enabled",True));trigger=dict(raw.get("trigger") or {});tt=str(trigger.get("type") or "").strip().lower()
 ...
 for raw_action in raw.get("actions") or []:
  ...
  action["type"]=atype
  if atype=="broadcast":action={"type":"broadcast","broadcast":normalize_broadcast(action.get("broadcast") or {})}
  elif atype=="instance":
  ...
  elif atype=="backup":action={"type":"backup","instance_id":_token(action.get("instance_id"),"instance_id")}
  elif atype=="content":
   assignment=action.get("assignment")
   if not isinstance(assignment,Mapping):raise AutomationValidationError("content action requires assignment")
   action={"type":"content","assignment":dict(assignment)}
  elif atype=="configuration":
   configuration=action.get("configuration")
   if not isinstance(configuration,Mapping):raise AutomationValidationError("configuration action requires configuration")
 ...
 if any(not isinstance(x,Mapping) for x in conditions):raise AutomationValidationError("conditions must be objects")
 identity={"rule_id":rule_id,"name":name,"enabled":enabled,"trigger":trigger,"conditions":conditions,"actions":actions,"cooldown_seconds":max(0,min(int(raw.get("cooldown_seconds") or 0),86400))}
